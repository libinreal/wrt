define(function(require) {
    ...
    //合同验签
    $('#zj-detail').on('click', '.order-confirm', function(e) {
        e.preventDefault();
        ...
        //获取订单签名数据
        Ajax.custom({
            url: config.server + '/bank/getConfirmData',
            data: {
                orderId: id//当前订单id
            },
            type: 'POST'
            ...
            Ajax.custom({
                url: config.server + '/bank/confirmContract',
                data: {
                ...
                that.addClass('disabled').text('订单已对账验签');
