...
{
    $smarty->assign('ur_here',      $_LANG['03_brand_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['04_brand_add'], 'href' => 'brand.php?act=add'));
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',     $_LANG['04_brand_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_brand_list'], 'href' => 'brand.php?act=list'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $img_path = 'data/brandlogo/'.$img_name;
     /*处理URL*/
    $site_url = sanitize_url( $_POST['site_url'] );
    /*插入数据*/
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order,brand_code,brand_pinyin) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_path', '$is_show', '$_POST[sort_order]','$_POST[brand_code]','$_POST[brand_pinyin]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order,brand_code ".
            "FROM " .$ecs->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);

    $smarty->assign('ur_here',     $_LANG['brand_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['03_brand_list'], 'href' => 'brand.php?act=list&' . list_link_postfix()));
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $img_path = 'data/brandlogo/'.$img_name;
    $param = "brand_name = '$_POST[brand_name]', brand_code='$_POST[brand_code]', site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]',brand_pinyin='".$_POST['brand_pinyin']."' ";
    if (!empty($img_name))
    {
        //有图片上传
        $param .= " ,brand_logo = '$img_path' ";
    }
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY sort_order ASC,brand_id DESC ";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('brand')." ORDER BY sort_order ASC,brand_id DESC ";
        }
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        $brand_logo = empty($rows['brand_logo']) ? '' :
            '<a href="../'.$rows['brand_logo'].'" target="_brank"><img src="images/picflag.gif" width="16" height="16" border="0" alt='.$GLOBALS['_LANG']['brand_logo'].' /></a>';
