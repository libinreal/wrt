 ...
 * 友情链接管理
 * $Author: xy $
 * $Id: links.php 17217 2014-09-11 xy $
*/
...
$exc = new exchange($ecs->table('links'), $db, 'id', 'title');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['add_link'], 'href' => 'links.php?act=add'));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('links_list.htm'), '',
...
{
    admin_priv('engineering_links');

    $smarty->assign('ur_here',     $_LANG['add_link']);
    $smarty->assign('action_link', array('href'=>'links.php?act=list', 'text' => $_LANG['list_link']));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');

    assign_query_info();
    $smarty->display('links_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("title", $link_name) == 0)
    {
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('links')." (title, url, icon, type, createAt, updateAt, userid) ".
                  "VALUES ('$link_name', '$link_url', '$link_logo', '$_POST[link_type]', '$add_time', '$add_time', '$userid')";
        $db->query($sql);

        /* 记录管理员操作 */
        admin_log($_POST['link_name'], 'add', 'links');
        ...
        $link[0]['href'] = 'links.php?act=add';

        $link[1]['text'] = $_LANG['back_list'];
        $link[1]['href'] = 'links.php?act=list';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取得友情链接数据 */
    $sql = "SELECT id, title, url, icon, type, createAt ".
           "FROM " .$ecs->table('links'). " WHERE id = '".intval($_REQUEST['id'])."'";
    $link_arr = $db->getRow($sql);

    /* 标记为图片链接还是文字链接 */
    if (!empty($link_arr['icon']))
    {
        $type      = 'img';
        $link_logo = $link_arr['icon'];
    }
    else
    {
        $type      = 'chara';
    ...
    $link_arr['title'] = sub_str($link_arr['title'], 250, false); // 截取字符串为250个字符避免出现非法字符的情况

    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['edit_link']);
    $smarty->assign('action_link', array('href'=>'links.php?act=list&' . list_link_postfix(), 'text' => $_LANG['list_link']));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        $img_up_info = @basename($image->upload_image($_FILES['link_img'], 'afficheimg'));
        $link_logo   = ", icon = ".'\''. DATA_DIR . '/afficheimg/'.$img_up_info.'\'';
    }
    elseif (!empty($_POST['url_logo']))
    {
        $link_logo = ", icon = '$_POST[url_logo]'";
    }
    else
    {
        /* 如果是文字链接, LOGO为链接的名称 */
    ...
    //如果要修改链接图片, 删除原来的图片
    $old_logo = $db->getOne("SELECT icon FROM " .$ecs->table('links'). " WHERE id = '$id'");
    if (!empty($img_up_info) || (!empty($_POST['url_logo']) && $_POST['url_logo'] != $old_logo))
    {
        //获取链子LOGO,并删除
    ...
    $updateat = gmtime();
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('links'). " SET ".
            "title = '$link_name', ".
            "url = '$link_url' ".
            $link_logo.','.
    ...
    $db->query($sql);
    /* 记录管理员操作 */
    admin_log($_POST['link_name'], 'edit', 'links');
    ...
    $link[0]['href'] = 'links.php?act=list&' . list_link_postfix();
...
{
    check_authz_json('engineering_links');

    $id        = intval($_POST['id']);
    $link_name = json_str_iconv(trim($_POST['val']));

    /* 检查链接名称是否重复 */
    if ($exc->num("title", $link_name, $id) != 0)
    {
        make_json_error(sprintf($_LANG['link_name_exist'], $link_name));
    }
    else
    {
        if ($exc->edit("title = '$link_name'", $id))
        {
            admin_log($link_name, 'edit', 'links');
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获取链子LOGO,并删除 */
    $link_logo = $exc->get_name($id, "icon");
    ...
    $exc->drop($id);
    clear_cache_files();
    admin_log('', 'remove', 'links');

    $url = 'links.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function get_links_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('links');
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            if ((strpos($rows['icon'], 'http://') === false) && (strpos($rows['icon'], 'https://') === false))
            {
                $rows['icon'] = "<img src='" .'../'.$rows['icon']. "' width=88 height=31 />";
            }
            else
            {
                $rows['icon'] = "<img src='".$rows['icon']."' width=88 height=31 />";
