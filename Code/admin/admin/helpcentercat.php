 ...
 * 商城公告分类管理程序
 * $Author: xy $
 * $Id: helpcentercat.php 2014-09-04 xy $
*/
...
{
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 3000);
    $articlecat = article_cat_list($catid, 0, false);

    foreach ($articlecat as $key => $cat)
    {
        $articlecat[$key]['type_name'] = $_LANG['type_name'][$cat['cat_type']];
    }
    $smarty->assign('ur_here',     $_LANG['02_helpcentercat_list']);
    //$smarty->assign('action_link', array('text' => $_LANG['helpcentercat_add'], 'href' => 'helpcentercat.php?act=add'));
    $smarty->assign('full_page',   1);
    $smarty->assign('articlecat',        $articlecat);

    assign_query_info();
    $smarty->display('helpcentercat_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('helpcentercat_list.htm'));
}
...
{
    /* 权限判断 */
    admin_priv('helpcenter_cat');
    $sql = "SELECT cat_id FROM " . $ecs->table('article_cat') . " WHERE cat_type = 3000 and parent_id = 0";
    $cat_id = $db->getOne($sql);
    $smarty->assign('cat_select',  article_cat_list($cat_id,$cat_id));
    $smarty->assign('ur_here',     $_LANG['helpcentercat_add']);
    $smarty->assign('action_link', array('text' => $_LANG['02_helpcentercat_list'], 'href' => 'helpcentercat.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('helpcentercat_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav,createAt,updateAt,user_id)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]', '$createat', '$createat', '$userid')";
    $db->query($sql);
    ...
    admin_log($_POST['cat_name'],'add','helpcentercat');

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'helpcentercat.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'helpcentercat.php?act=list';
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $updateat = gmtime();
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', updateAt='$updateat', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        $link[0]['href'] = 'helpcentercat.php?act=list';
...
{
    check_authz_json('helpcenter_cat');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT cat_type,parent_id FROM " . $ecs->table('article_cat') . " WHERE cat_id = '$id'";
    $cat_type = $db->getRow($sql);
    if (($cat_type['cat_type'] >= 1000 && $cat_type['cat_type']< 3000) || ($cat_type['cat_type'] == 3000 && $cat_type['parent_id'] == 0 ))
    {
        /* 系统保留分类，不能删除 */
    ...
    $url = 'helpcentercat.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
