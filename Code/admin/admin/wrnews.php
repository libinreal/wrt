...
if($_REQUEST['act'] == 'list_arr') {
    ...
    $smarty->assign('ur_here', "内容管理");
    $smarty->assign('full_page',        1);

    assign_query_info();
    $smarty->display('news_list_arr.htm');
}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $filter = array();
    $smarty->assign('cat_select',  article_cat_list($catid));
    $smarty->assign('ur_here',      $_LANG['03_wrnews_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['wrnews_add'], 'href' => 'wrnews.php?act=add'));
    $smarty->assign('full_page',    1);
    ...
    assign_query_info();
    $smarty->display('wrnews_list.htm');
}
...
{
    check_authz_json('wrnews_manage');
    ...
    make_json_result($smarty->fetch('wrnews_list.htm'), '',
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 1000);
    $smarty->assign('cat_select',  article_cat_list($catid));
    $smarty->assign('ur_here',     $_LANG['wrnews_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_wrnews_list'], 'href' => 'wrnews.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('wrnews_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    if($_POST[cat_type] == 3000){
        ...
        if($_POST[article_type] == 1){
            ...
            $sql = "update ".$GLOBALS['ecs']->table('article')." set article_type=0 WHERE cat_id=".$_POST['article_cat']." ";
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,imgurl,cat_type,createAt,updateAt,user_id,source) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]', '$imgurl', '$_POST[cat_type]', '$createat', '$createat', '$userid','$_POST[source]')";
    $db->query($sql);
    ...
    $link[0]['href'] = 'wrnews.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'wrnews.php?act=list';

    admin_log($_POST['title'],'add','wrnews');
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('article',     $article);
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 1000);
    $smarty->assign('cat_select',  article_cat_list($catid, $article['cat_id']));
    $smarty->assign('ur_here',     $_LANG['article_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['03_wrnews_list'], 'href' => 'wrnews.php?act=list&' . list_link_postfix()));
...
if ($_REQUEST['act'] =='update')
{
    ...
    is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]',source='$_POST[source]',
    ...
    link='$_POST[link_url]', description = '$_POST[description]', updateAt='$updateat', imgurl = '$imgurl', cat_type = '$_POST[cat_type]'", $_POST['id']))
    {
        ...
        $link[0]['href'] = 'wrnews.php?act=list&' . list_link_postfix();

        $note = sprintf($_LANG['articleedit_succeed'], stripslashes($_POST['title']));
        admin_log($_POST['title'], 'edit', 'wrnews');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'wrnews.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'wrnews.php?act=list');
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        $where .= "AND ac.cat_type >= 1000 and ac.cat_type < 2000 ";
        /* 文章总数 */
