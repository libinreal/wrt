...
if($_REQUEST['act'] == 'list_arr') {
    ...
    $smarty->assign('ur_here', "基数数据管理");
    $smarty->assign('full_page',        1);

    assign_query_info();
    $smarty->display('base_list_arr.htm');
}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',      $_LANG['01_category_list']);
    $smarty->assign('action_link',  array('href' => 'category.php?act=add', 'text' => $_LANG['02_category_add']));
    $smarty->assign('full_page',    1);
...
if ($_REQUEST['act'] == 'add')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',      $_LANG['02_category_add']);
    $smarty->assign('action_link',  array('href' => 'category.php?act=list', 'text' => $_LANG['01_category_list']));
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat['code'] = !empty($_POST['code']) ? $_POST['code'] : '';
    $cat['wcode'] = !empty($_POST['wcode']) ? $_POST['wcode'] : '';

    if($cat['parent_id']==0) {
        $cat['cat_level'] = 1;
    }else {
        $sql = "select cat_level from ".$GLOBALS['ecs']->table('category')." where parent_id=".$cat['cat_id']."";
        $cat['cat_level'] = $GLOBALS['db']->getOne($sql);
        $cat['cat_level'] = $cat['cat_level'] + 1;
    }
    ...
    {
        $sql = "SELECT  * FROM ".$GLOBALS['ecs']->table('category')." WHERE cat_id=(SELECT MAX(cat_id) as cat_id FROM ".$GLOBALS['ecs']->table('category').")";
        $res = $GLOBALS['db']->getRow($sql);
        // 如果level为2 也就是二级分类的时候将插入物料类型
        //if($res['cat_level']==2) {
        //    $sql2 ="INSERT INTO ".$GLOBALS['ecs']->table('goods_type')."(cat_name,enabled,attr_group,code) VALUES('".$res['cat_name']."','1','','".$cat['code']."')";
        //    $db->query($sql2);
        //}
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['01_category_list'], 'href' => 'category.php?act=list'));
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "select cat_level from ".$GLOBALS['ecs']->table('category')." where cat_id=".$cat_id."";
    $cat['cat_level'] = $GLOBALS['db']->getOne($sql);
    $cat['cat_level'] = $cat['cat_level'] ;
...
//    exit();
    ...
    if ($db->autoExecute($ecs->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        ...
        // 如果level为2 也就是二级分类的时候将插入物料类型
        //if($res['cat_level']==2) {
        //    $sql2 ="UPDATE ".$GLOBALS['ecs']->table('goods_type')." set cat_name='".$cat['cat_name']."', code='".$cat['code']."' where cat_name='".$old_cat_name."'";
        //    $db->query($sql2);
        //}
        /* 更新分类信息成功 */
        clear_cache_files(); // 清除缓存
