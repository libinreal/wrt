...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('attribute_list.htm'), '',array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
//-- 添加/编辑属性
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        if($attr['attr_input_type']==1) {
            ...
            $attr_code = str_replace("\n", "~", $attr['attr_code']);
            $attrCode = explode('~',$attr_code);
            foreach($attrVal as $k=>$v) {
                $arrAttr .= stripcslashes(trim($v)).",".$attrCode[$k]."";
            }
            $attr['attr_values'] = $arrAttr;
        }
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $sql = "select code from ".$GLOBALS['ecs'] -> table('goods_type')." where cat_id=".$cat_id."";
    $cat_code = $GLOBALS['db']->getOne($sql);
    if($_POST['attr_input_type']==1) {
        $attr_values = isset($_POST['attr_values']) ? $_POST['attr_values'] : '';
        $attr_val = str_replace("\r\n", "~", $attr_values);
        $attrArr = explode('~',$attr_val);
        $attrValues =  '';
        $attrCodes = '';
