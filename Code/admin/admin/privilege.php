...
if ($_REQUEST['act'] == 'login')
{
    ...
    $smarty->assign('DataYear',date("Y",time()));
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $_POST['role'] = isset($_POST['role']) ? trim($_POST['role']) : '';
    $sql="SELECT `ec_salt` FROM ". $ecs->table('admin_user') ."WHERE user_name = '" . $_POST['username']."'";
    $ec_salt =$db->getOne($sql);
    if(!empty($ec_salt))
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt,role_id".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5(md5($_POST['password']).$ec_salt) . "' AND role_id='".$_POST['role']."'";
    }
    else
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5($_POST['password']) . "' AND role_id='".$_POST['role']."'";
    }
    $row = $db->getRow($sql);
    if ($row)
    {
        ...
        if (isset($_POST['remember']))
        {
            ...
            setcookie('ECSCP[role_id]',   $row['role_id'],                            $time);
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $link[0]['href'] = 'privilege.php?act=add';
