...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['03_notice_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['notice_add'], 'href' => 'notice.php?act=add'));
...
{
    check_authz_json('notice_manage');
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 2000);
    $smarty->assign('cat_select',  article_cat_list($catid));
    $smarty->assign('ur_here',     $_LANG['notice_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_notice_list'], 'href' => 'notice.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('notice_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $link[0]['href'] = 'notice.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'notice.php?act=list';

    admin_log($_POST['title'],'add','notice');
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['03_notice_list'], 'href' => 'notice.php?act=list&' . list_link_postfix()));
...
if ($_REQUEST['act'] =='update')
{
    ...
    link='$_POST[link_url]', description = '$_POST[description]', updateAt='$updateat', imgurl = '$imgurl', cat_type = '$_POST[cat_type]'", $_POST['id']))
    {
        ...
        $link[0]['href'] = 'notice.php?act=list&' . list_link_postfix();

        $note = sprintf($_LANG['articleedit_succeed'], stripslashes($_POST['title']));
        admin_log($_POST['title'], 'edit', 'notice');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'notice.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'notice.php?act=list');
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        $where .= "AND ac.cat_type >= 2000 and ac.cat_type < 3000 ";
        /* 文章总数 */
