...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.cat_id,a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
...
function getInsertCategory() {
    $sql = " SELECT p.cat_id,p.code,p.cat_name FROM ".$GLOBALS['ecs']->table('category')." p ".
           " WHERE p.cat_id NOT IN ( ".
                "SELECT category.parent_id FROM ".$GLOBALS['ecs']->table('category')." ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
            $html .= "<input name='goods_type' type='hidden' value='".$val['cat_id']."'/>";
            $html .= '<select name="attr_value_list[]">';
            $html .= '<option value="default">' .$GLOBALS['_LANG']['select_please']. '</option>';
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        //$sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g WHERE suppliers_id=".$_SESSION['suppliers_id']." AND user_id=".$_SESSION['admin_id']." AND is_delete='$is_delete' $where";
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g INNER JOIN ".$GLOBALS['ecs']->table('region')." AS rg ON g.area_id=rg.region_id WHERE  g.user_id=".$_SESSION['admin_id']." AND g.is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

//        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price,market_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
