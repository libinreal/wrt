...
function create_html_editor($input_name, $input_value = '')
{
   ...
   <textarea id=\"$input_name\" name=\"$input_name\" style='width:90%;height:400px;'>$input_value</textarea>
    ";
 ...
 * @param   string  input_value 输入框值
 */
function create_html_sales_editor($input_name, $input_value = '')
{
    global $smarty;
   ...
   <textarea id=\"$input_name\" name=\"$input_name\" style='width:90%;height:400px;'>$input_value</textarea>
    ";
    $smarty->assign('SalesFCKeditor', $kindeditor);
}
/**
 * 生成售后信息
 * @param   string  input_name  输入框名称
 * @param   string  input_value 输入框值
 */
function create_html_exchange_spec_editor($input_name, $input_value = '')
{
    global $smarty;
   ...
   <textarea id=\"$input_name\" name=\"$input_name\" style='width:90%;height:400px;'>$input_value</textarea>
    ";
    $smarty->assign('specFCKeditor', $kindeditor);
}
/**
 * 使用说明
 * @param   string  input_name  输入框名称
 * @param   string  input_value 输入框值
 */
function create_html_explain_spec_editor($input_name, $input_value = '')
{
    global $smarty;
    $kindeditor="<script charset='utf-8' src='../includes/kindeditor/kindeditor-min.js'></script>
    <script charset='utf-8' src='../includes/kindeditor/lang/zh_CN.js'></script>
   ...
   <textarea id=\"$input_name\" name=\"$input_name\" style='width:90%;height:400px;'>$input_value</textarea>
    ...
    $smarty->assign('explain_FCKeditor', $kindeditor);
}

function create_html_goods_desc_editor($input_name, $input_value = '') {
    global $smarty;
    $kindeditor="<script charset='utf-8' src='../includes/kindeditor/kindeditor-min.js'></script>
    <script charset='utf-8' src='../includes/kindeditor/lang/zh_CN.js'></script>
    <script type='text/javascript'>
   ...
   <textarea id=\"$input_name\" name=\"$input_name\" style='width:90%;height:400px;'>$input_value</textarea>
    ...
    $editor->Value      = $input_value;
    $FCKeditor = $editor->CreateHtml();*/
    $smarty->assign('goods_brief_FCKeditor', $kindeditor);
}
/**
...
/**
 * 取得积分商品列表：用于把商品添加到组合、关联类、赠品类
 * @param object $filter 过滤条件
 * @return mixed
 */
function get_exchange_goods_list($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_sql($filter); // 取得过滤条件

    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_name, shop_price '. 'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS g ' . $where . 'LIMIT 50';
    $row = $GLOBALS['db']->getAll($sql);
 ...
 * @return  string
 */
function exchange_goods_type_list($selected)
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('exchange_goods_type') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
 ...
 * @return  array
 */
function get_exchange_attr_groups($cat_id)
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['ecs']->table('exchange_goods_type') . " WHERE cat_id='$cat_id'";
 ...
 * @return  boolean
 */
function exchange_cat_exists($cat_name, $parent_cat, $exclude = 0)
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('exchange_category').
        " WHERE parent_id = '$parent_cat' AND cat_name = '$cat_name' AND cat_id<>'$exclude'";
    return ($GLOBALS['db']->getOne($sql) > 0) ? true : false;
}
