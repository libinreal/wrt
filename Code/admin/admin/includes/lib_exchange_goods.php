 ...
 */
function handle_exchange_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    $goods_attr_id = array();
    ...
    foreach ($id_list AS $key => $id)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . $GLOBALS['ecs']->table('exchange_goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = $GLOBALS['db']->getOne($sql);
        if (!empty($result_id))
        {
            $sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_goods_attr') . "
        ...
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('exchange_goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
 ...
 * @return  void
 */
function handle_exchange_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    /* 是否处理缩略图 */
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
    if(is_array($image_descs)) {
        foreach ($image_descs AS $key => $img_desc)
        {
            ...
            if ($flag)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('exchange_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
                $GLOBALS['db']->query($sql);
                /* 不保留商品原图的时候删除原图 */
                if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
                {
                    $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('exchange_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                    @unlink('../' . $img_original);
                }
            }
 ...
 * @return  bool
 */
function update_exchange_goods($goods_id, $field, $value)
{
    if ($goods_id)
    {
        /* 清除缓存 */
 ...
 * @return  void
 */
function delete_exchange_goods($goods_id)
{
    if (empty($goods_id))
    {
        return;
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . $GLOBALS['ecs']->table('exchange_goods') .
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('exchange_goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('exchange_products') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('exchange_gallery') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
...
//    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('exchange_goods_attr') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
 ...
 * @return  string  唯一的货号
 */
function generate_exchange_goods_sn($goods_id)
{
    $goods_sn = $GLOBALS['_CFG']['sn_prefix'] . str_repeat('0', 6 - strlen($goods_id)) . $goods_id;

    $sql = "SELECT goods_sn FROM " . $GLOBALS['ecs']->table('exchange_goods') .
...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id))
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('exchange_goods') ."
 ...
 */
function get_exchange_attr_list($cat_id, $goods_id = 0)
{
    if (empty($cat_id))
    {
        return array();
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
            "FROM " .$GLOBALS['ecs']->table('exchange_attribute'). " AS a ".
            "LEFT JOIN " .$GLOBALS['ecs']->table('exchange_goods_attr'). " AS v ".
 ...
 * @return  array
 */
function get_exchange_goods_type_specifications()
{
    // 查询
    $sql = "SELECT DISTINCT cat_id
            FROM " .$GLOBALS['ecs']->table('exchange_attribute'). "
            WHERE attr_type = 1";
    $row = $GLOBALS['db']->GetAll($sql);
 ...
 * @return  string
 */
function exchange_build_attr_html($cat_id, $goods_id = 0)
{
    $attr = get_exchange_attr_list($cat_id, $goods_id);
 ...
 * @return  array
 */
function exchange_goods_list($is_delete, $real_goods=1, $conditions = '')
{
    /* 过滤条件 */
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('exchange_goods'). " AS g WHERE is_delete='$is_delete' $where";
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    ...
                    " FROM " . $GLOBALS['ecs']->table('exchange_goods') . " AS g WHERE is_delete='$is_delete' $where" .
 ...
 */
function check_exchange_goods_product_exist($goods_id, $conditions = '')
{
    if (empty($goods_id))
    {
    ...
    $sql = "SELECT goods_id
            FROM " . $GLOBALS['ecs']->table('exchange_products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions . "
            LIMIT 0, 1";
 ...
 * @return      array
 */
function get_exchange_goods_specifications_list($goods_id)
{
    if (empty($goods_id))
    {
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . $GLOBALS['ecs']->table('exchange_goods_attr') . " AS g
                LEFT JOIN " . $GLOBALS['ecs']->table('exchange_attribute') . " AS a
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('exchange_products'). " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['ecs']->table('exchange_products') . " AS g
 ...
 * @return  array
 */
function get_exchange_product_info($product_id, $filed = '')
{
    $return_array = array();
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['ecs']->table('exchange_products') . " WHERE product_id = '$product_id'";
 ...
 */
function check_exchange_goods_specifications_exist($goods_id)
{
    $goods_id = intval($goods_id);

    $sql = "SELECT COUNT(a.attr_id)
            FROM " .$GLOBALS['ecs']->table('exchange_attribute'). " AS a, " .$GLOBALS['ecs']->table('exchange_goods'). " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
    ...
    if (empty($product_id))
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['ecs']->table('exchange_products') ."
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['ecs']->table('exchange_goods')."WHERE goods_sn='$product_sn'";
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    {
        case 'goods':
            $img_name = $goods_id . '_G_exchange_' . $rand_name;
            break;
        case 'goods_thumb':
            $img_name = $goods_id . '_thumb_G_exchange_' . $rand_name;
            break;
        case 'gallery':
            $img_name = $goods_id . '_P_exchange_' . $rand_name;
            break;
        case 'gallery_thumb':
            $img_name = $goods_id . '_thumb_P_exchange_' . $rand_name;
            break;
    }
    if ($position == 'source')
    {
