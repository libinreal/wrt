 ...
 * 管理中心新闻处理程序文件
 * $Author: xy $
 * $Id: helpcenter.php 2014-09-03 xy $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['03_helpcenter_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['helpcenter_add'], 'href' => 'helpcenter.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('helpcenter_list.htm');
}
...
{
    check_authz_json('helpcenter_manage');
    ...
    make_json_result($smarty->fetch('helpcenter_list.htm'), '',
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 3000);
    $smarty->assign('cat_select',  article_cat_list($catid));
    $smarty->assign('ur_here',     $_LANG['helpcenter_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_helpcenter_list'], 'href' => 'helpcenter.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('helpcenter_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $link[0]['href'] = 'helpcenter.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'helpcenter.php?act=list';

    admin_log($_POST['title'],'add','helpcenter');
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['03_helpcenter_list'], 'href' => 'helpcenter.php?act=list&' . list_link_postfix()));
...
if ($_REQUEST['act'] =='update')
{
    ...
    $updateat = gmtime();
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]',source='$_POST[source]',
    ...
    link='$_POST[link_url]', description = '$_POST[description]', updateAt='$updateat', imgurl = '$imgurl', cat_type = '$_POST[cat_type]'", $_POST['id']))
    {
        ...
        $link[0]['href'] = 'helpcenter.php?act=list&' . list_link_postfix();

        $note = sprintf($_LANG['articleedit_succeed'], stripslashes($_POST['title']));
        admin_log($_POST['title'], 'edit', 'helpcenter');
...
elseif ($_REQUEST['act'] == 'edit_title')
{
    ...
    else
    {
        ...
        {
            clear_cache_files();
            admin_log($title, 'edit', 'helpcenter');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'helpcenter.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    /* 清除缓存 */
    clear_cache_files();
    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'helpcenter.php?act=list');
