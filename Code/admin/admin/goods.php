...
if($_REQUEST['act'] == 'list_arr') {
    ...
    $smarty->assign('ur_here', $_LANG['01_goods_list']);
    $smarty->assign('full_page',        1);

    assign_query_info();
    $smarty->display('goods_list_arr.htm');
}
//-- 商品列表，商品回收站
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        /* 商品信息 */
        $sql = "SELECT b.brand_name,g.* FROM ".$ecs->table('goods')." AS g INNER JOIN ".$ecs->table('brand')." AS b ON g.brand_id=b.brand_id WHERE g.goods_id=".$_REQUEST[goods_id]."";
        $goods = $db->getRow($sql);
        if (empty($goods) === true)
        {
            /* 默认值 */
    ...
    create_html_explain_spec_editor('goods_explain', $goods['goods_explain']);
    create_html_goods_desc_editor('goods_brief', $goods['goods_brief']);
    $area = get_regions(1,1);
    /* 模板赋值 */
    $cat_list2 = cat_list(0, $goods['cat_id'],false);
    // 获取可以添加商品的分类
    //$insertCate =    getInsertCategory();
    $smarty -> assign('insertCategory',getInsertCategory());
    // 地区列表
    $smarty->assign('areas',    $area);
    $smarty->assign('ur_here', $is_add ? $_LANG['02_goods_add'] : $_LANG['edit_goods'] );
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $goods_explain = isset($_POST['goods_explain']) ? $_POST['goods_explain'] : '';
    $sales = isset($_POST['sales']) ? $_POST['sales'] : '';
    $shiplocal = isset($_POST['shiplocal']) ? $_POST['shiplocal'] : '';
    $wCode = '';
    //取四级分类的wcode
    $sqlcode = "select cat_name,code,wcode,parent_id,cat_level from ".$GLOBALS['ecs']->table('category')." where cat_id=".$catgory_id."";
    $cat_code = $GLOBALS['db']->getRow($sqlcode);
    //取三级分类的wcode
    if(!empty($cat_code['parent_id'])) {
        $sql_cat_parent_id = "select parent_id,wcode from ".$GLOBALS['ecs'] -> table('category')." where cat_id=".$cat_code['parent_id']."";
    } else {
        $sql_cat_parent_id = "select parent_id,wcode from ".$GLOBALS['ecs'] -> table('category')." where cat_id=".$catgory_id."";
    }
    $resParent = $GLOBALS['db']->getRow($sql_cat_parent_id);
    if(!empty($resParent['parent_id'])){
        // 取二级分类的wcode
        $sql_level_2 = "select wcode from ".$GLOBALS['ecs'] -> table('category') ."where cat_id=".$resParent['parent_id']."";
    }else {
        $sql_level_2 = "select wcode from ".$GLOBALS['ecs'] -> table('category') ."where cat_id=".$catgory_id."";
    }
    $level2wCode = $GLOBALS['db'] -> getOne($sql_level_2);
    $wCode = stripcslashes(trim($level2wCode.$resParent['wcode'].$cat_code['wcode']));
    if(strlen($wCode)==4) {$wCode = $wCode."00";}
    if(strlen($wCode)==2) {$wCode = $wCode."0000";}
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        // 处理物料编码问题
        $sql_attr = "SELECT attr.attr_values,attr.attr_code FROM ". $GLOBALS['ecs'] -> table('goods_type')." as type ".
            " INNER JOIN ". $GLOBALS['ecs'] ->table('attribute') ." as attr ON type.cat_id=attr.cat_id ".
            " WHERE type.cat_id=".$goods_type." order by attr.sort_order asc limit 3";
        $res_attr = $GLOBALS['db'] -> getAll($sql_attr);

        foreach($res_attr as $key => &$attr_val) {
            $attr_val['attr_values'] = str_replace("\r\n", "~", $attr_val['attr_values']);
            $attr_val['attr_code'] = str_replace("\r\n", "~", $attr_val['attr_code']);
            $attr_val['attr_values'] = explode('~',$attr_val['attr_values']);
            $attr_val['attr_code'] = explode('~',$attr_val['attr_code']);
        }
        foreach($res_attr as $key=>&$val) {
            foreach($val['attr_code'] as $k=>&$v){
                $val[$v]=$val['attr_values'][$k];
            }
            unset($val['attr_values']);
            unset($val['attr_code']);
        }
        $attr_value_list = $_POST['attr_value_list'];
        $w_attr_code = '';
        foreach($res_attr as $k => $attrVal) {
            if($k < 3) {
                foreach($attrVal as $key=>$value) {
                    if(in_array($attr_value_list[$k],$attrVal)) {
                        if($value==$attr_value_list[$k]) {
                            $w_attr_code+=$key;
                        }
                    }
                }
        ...
        $strLens = strlen($w_attr_code);
        if($strLens !=6) {
            $lenStr = 6 - $strLens;
            if($lenStr==1) $str = '0';
            if($lenStr==2) $str = '00';
            if($lenStr==3) $str = '000';
            if($lenStr==4) $str = '0000';
            if($lenStr==5) $str = '00000';
            if($lenStr==6) $str = '000000';
            $w_attr_code = $str.$w_attr_code;
        }
        $w_attr_code_info = $wCode.$w_attr_code;
    }
    /*if($cat_code['cat_level']!=4) {
        sys_msg(sprintf($_LANG['invalid_goods_category']));
    }*/
    /* 入库 */
    if ($is_insert)
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
            ...
            " suppliers_id,area_id,sales,goods_explain,user_id,cat_code,wcode,shiplocal,is_real)" .
            ...
            " '$is_alone_sale', $is_shipping,  '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', ".
            " '$rank_integral','$_SESSION[suppliers_id]','$area','".$sales."','".$goods_explain."','".$_SESSION['admin_id']."'," .
            " '$cat_code[code]','$w_attr_code_info','".$shiplocal."','1')";
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
        ...
        $sql .= " keywords = '$_POST[keywords]', " .
            ...
            " cat_code='".$cat_code['code']."' ,".
            " wcode='$w_attr_code_info', ".
            " shiplocal='$shiplocal' ".
    ...
    if($is_insert) {
        ...
        $gmTime = gmtime();
        $sql2 = "INSERT INTO ". $GLOBALS['ecs'] -> table('goods_price_log')."(goodsId,wcode,price,createAt,brandId,brandName,supplierId,vscope,cat_code) ".
            " VALUES('$goods_id','$w_attr_code_info','$shop_price',$gmTime,'$brand_id','$brand_name','$_SESSION[suppliers_id]','0','$cat_code[code]')";
        $GLOBALS['db']->query($sql2);
    } else {
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('goods_price_log')." WHERE goodsId=$goods_id ORDER BY id DESC LIMIT 1 ";
        $priceLog = $GLOBALS['db'] -> getRow($sql);
        if(!empty($priceLog)) { // 如果价格变动表存在 则比较
            if($w_attr_code_info==$priceLog['wcode'] && $brand_id==$priceLog['brandId']) {
                $sql = "SELECT brand_name FROM ".$GLOBALS['ecs']->table('brand')." WHERE brand_id=$brand_id";
                $brand_name = $GLOBALS['db']->getOne($sql);
                $gmTime = gmtime();
                if($priceLog['price']>$shop_price) {
                    $vscope = $priceLog['price'] - $shop_price;
                }else {
                    $vscope = $shop_price-$priceLog['price'];
                }
                $sql2 = "INSERT INTO ". $GLOBALS['ecs'] -> table('goods_price_log')."(goodsId,wcode,price,createAt,brandId,brandName,supplierId,vscope,cat_code) ".
                    " VALUES('$goods_id','$w_attr_code_info','$shop_price',$gmTime,'$brand_id','$brand_name','$_SESSION[suppliers_id]','$vscope','$cat_code[code]')";
                $GLOBALS['db']->query($sql2);
            }
        }else {
            $sql = "SELECT brand_name FROM ".$GLOBALS['ecs']->table('brand')." WHERE brand_id=$brand_id";
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        $sql = "SELECT g.*, a.attr_type
                FROM " . $ecs->table('goods_attr') . " AS g LEFT JOIN " . $ecs->table('attribute') . " AS a ON a.attr_id = g.attr_id
...
elseif ($_REQUEST['act'] == 'goods_type') {
    check_authz_json('goods_manage');
    $categoryId = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
    $goods_id   = empty($_REQUEST['goodsId']) ? 0 : intval($_REQUEST['goodsId']);
    $sql = "select code from ".$GLOBALS['ecs']->table('category')." where cat_id=".$categoryId."";
    $catCode = $GLOBALS['db']->getOne($sql);
    $sql2 = "select cat_id from ".$GLOBALS['ecs']->table('goods_type')." where code='".$catCode."'";
    $goods_type = $GLOBALS['db']->getOne($sql2);
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
    ...
    else
    {
        if ($exc->edit("shop_price = '$goods_price', market_price = '$price_rate', last_update=" .gmtime(), $goods_id))
        {
            ...
            $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('goods_price_log')." WHERE goodsId=$goods_id ORDER BY id DESC LIMIT 1 ";
            $priceLog = $GLOBALS['db'] -> getRow($sql);
            if($priceLog['price']>$goods_price) {
                $vscope = $priceLog['price'] - $goods_price;
            }else {
                $vscope = $goods_price-$priceLog['price'];
            }
            $sql2 = " SELECT g.wcode,g.cat_code,g.suppliers_id,g.goods_id,g.brand_id,b.brand_name  FROM ".$GLOBALS['ecs']->table('goods')."  AS g ".
                " INNER JOIN ".$GLOBALS['ecs']->table('brand')." as b ON g.brand_id=b.brand_id ".
                " LEFT JOIN ".$GLOBALS['ecs']->table('suppliers')." AS su ON g.suppliers_id=su.suppliers_id ".
                " WHERE g.goods_id=$goods_id";
            $arr = $GLOBALS['db']->getRow($sql2);

            $gmTime = gmtime();
            if(!empty($priceLog)) {
                $sql3 = "INSERT INTO ". $GLOBALS['ecs'] -> table('goods_price_log')."(goodsId,wcode,price,createAt,brandId,brandName,supplierId,vscope,cat_code) ".
                    " VALUES('$goods_id','".$arr['wcode']."','$goods_price',$gmTime,'".$arr['brand_id']."','".$arr['brand_name']."','".$arr['suppliers_id']."','$vscope','".$arr['cat_code']."')";
            } else {
                $sql3 = "INSERT INTO ". $GLOBALS['ecs'] -> table('goods_price_log')."(goodsId,wcode,price,createAt,brandId,brandName,supplierId,vscope,cat_code) ".
                    " VALUES('$goods_id','".$arr['wcode']."','$goods_price',$gmTime,'".$arr['brand_id']."','".$arr['brand_name']."','".$arr['suppliers_id']."','0','".$arr['cat_code']."')";
            }
...
elseif ($_REQUEST['act'] == 'edit_market_price') {
    ...
    if ($market_price < 0 || $market_price == 0 && $_POST['val'] != "$market_price")
    {
        make_json_error($_LANG['shop_price_invalid']);
    }
    else
    {
        if ($exc->edit("market_price = '$market_price', last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
            make_json_result(number_format($market_price, 2, '.', ''));
        }
    }
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'brands') {
    admin_priv('goods_manage'); // 检查权限
    $code = $_REQUEST['code'];
    $sql = "SELECT brand_name, brand_id from ".$GLOBALS['ecs']->table('brand')." where brand_pinyin like '".$code."%'";
