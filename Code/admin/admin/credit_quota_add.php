 ...
 * 信用额度追加申请程序文件
 * $Author: xy $
 * $Id: credit_quota_add.php 2014-09-15 xy $
 */
...
if ($_REQUEST['act'] == 'list')
{
    /* 取得过滤条件 */
    $filter = array();
    $smarty->assign('ur_here',      $_LANG['credit_quotaadd_list']);
    $type_list = array(1=>'订单相关', 2=>'售后相关', 3=>'投诉与建议');
    $smarty->assign('type_list',     $type_list);
    $smarty->assign('full_page',    1);
    $smarty->assign('filter',       $filter);

    $credit_quotaadd_list = get_credit_quotaaddlist();

    $smarty->assign('credit_quotaadd_list',    $credit_quotaadd_list['arr']);
    $smarty->assign('filter',          $credit_quotaadd_list['filter']);
    $smarty->assign('record_count',    $credit_quotaadd_list['record_count']);
    $smarty->assign('page_count',      $credit_quotaadd_list['page_count']);
    $sort_flag  = sort_flag($credit_quotaadd_list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);

    assign_query_info();
    $smarty->display('credit_quotaadd_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('credit_quotaadd_list.htm'), '',
        array('filter' => $credit_quotaadd_list['filter'], 'page_count' => $credit_quotaadd_list['page_count']));
}
...
elseif($_REQUEST['act'] == 'info'){
    ...
    {
        $info = get_credit_quotaadd_info();
    }

    $smarty->assign('ur_here',      $_LANG['credit_quotaadd_info']);
    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'credit_quota_add.php?act=list', 'text' => $_LANG['credit_quotaadd_list']);
    $smarty->assign('action_link',  $action_link);
    $smarty->assign('info',    $info);

    assign_query_info();
    $smarty->display('credit_quotaadd_info.htm');
}
...
{
    /* 权限判断 */
    admin_priv('credit_quota_add_state');
...
{
    check_authz_json('credit_quota_add_state');

    $id    = intval($_POST['id']);
    $remark = addslashes($_POST['remark']);
    if ($exc->edit("remark = '$remark'", $id)){
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'credit_quota_add.php?act=info&comid=' .$id ;
...
/* 获得定制列表 */
function get_credit_quotaaddlist()
{
    $result = get_filter();
    if ($result === false)
    {
...
/* 获得详细信息 */
function get_credit_quotaadd_info(){
    $result = get_filter();
    if ($result === false)
    {
        $filter = array();
