...
require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/lib_exchange_goods.php');
include_once(ROOT_PATH . '/includes/cls_image.php');
$image = new cls_image($_CFG['bgcolor']);
$exc = new exchange($ecs->table('exchange_goods'), $db, 'goods_id', 'goods_name');
//顶部菜单导航
if($_REQUEST['act'] == 'list_arr') {
    admin_priv('order_view');
    $smarty->assign('ur_here', $_LANG['03_exchange_goods']);
    $smarty->assign('full_page',        1);

    assign_query_info();
    $smarty->display('exchange_list_arr.htm');
}
//-- 商品列表，商品回收站
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    admin_priv('exchange_goods_manage');
    ...
    /* 模板赋值 */
    $goods_ur = array('' => $_LANG['03_exchange_goods']);
    $ur_here = ($_REQUEST['act'] == 'list') ? $goods_ur[$code] : $_LANG['07_exchange_goods_trash'];
    $smarty->assign('ur_here', $ur_here);

    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'exchange_goods.php?act=list', 'text' => $_LANG['03_exchange_goods']);
    $smarty->assign('action_link',  $action_link);
    $smarty->assign('code',     $code);
    $smarty->assign('cat_list',     exchangeCategoryList(0, $cat_id));
    ...
    $goods_list = exchange_goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1);
    ...
    /* 获取商品类型存在规格的类型 */
    $specifications = get_exchange_goods_type_specifications();
    $smarty->assign('specifications', $specifications);

    /* 显示商品列表页面 */
    assign_query_info();
    $htm_file = ($_REQUEST['act'] == 'list') ? 'exchange_goods_list.htm' : (($_REQUEST['act'] == 'trash') ? 'exchange_goods_trash.htm' : 'exchange_goods_list.htm');
...
//    else
//    {
        admin_priv('exchange_goods_manage'); // 检查权限
//    }
    ...
    if ($is_add)
    {
        ...
        /* 属性 */
        $sql = "DELETE FROM " . $ecs->table('exchange_goods_attr') . " WHERE goods_id = 0";
        $db->query($sql);
    ...
    {
        /* 商品信息 */
        $sql = "SELECT * FROM " . $ecs->table('exchange_goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods = $db->getRow($sql);
        ...
        $_attribute = get_exchange_goods_specifications_list($goods['goods_id']);
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price " .
                "FROM " . $ecs->table('exchange_goods_attr') .
                " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($ecs->table('exchange_goods_attr'), addslashes_deep($row), 'INSERT');
            }
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $ecs->table('exchange_gallery') . " WHERE goods_id = '$goods[goods_id]'";
    ...
    create_html_sales_editor('sales', $goods['sales']);
    create_html_exchange_spec_editor('spec',$goods['spec']);
    /* 模板赋值 */
    $smarty->assign('code',    $code);
    $smarty->assign('ur_here', $is_add ? (empty($code) ? $_LANG['04_exchange_goods_add'] : $_LANG['51_virtual_card_add']) : ($_REQUEST['act'] == 'edit' ? $_LANG['edit_exchange_goods'] : $_LANG['copy_exchange_goods']));
    $smarty->assign('action_link', list_link($is_add, $code));
    $smarty->assign('goods', $goods);
    $smarty->assign('cat_list', exchangeCategoryList(0, $goods['cat_id']));
    //商品品牌
    //$smarty->assign('brand_list', get_brand_list());
    // 重量单位换算
    ...
    $smarty->assign('goods_type_list', exchange_goods_type_list($goods['goods_type']));
    $smarty->assign('gd', gd_version());
    $smarty->assign('thumb_width', $_CFG['thumb_width']);
    $smarty->assign('thumb_height', $_CFG['thumb_height']);
    $smarty->assign('goods_attr_html', exchange_build_attr_html($goods['goods_type'], $goods['goods_id']));
...
//    }
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('exchange_goods') .
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
        ...
        if(is_array($_FILES['img_url']['error'])) {
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        if ($_REQUEST['goods_id'] > 0)
        {
            ...
            $sql = "SELECT goods_thumb, goods_img, original_img " .
                " FROM " . $ecs->table('exchange_goods') ." WHERE goods_id = '$_REQUEST[goods_id]'";
            $row = $db->getRow($sql);
    ...
    if (empty($_POST['goods_sn']))
    {
        ...
        $goods_sn   = generate_exchange_goods_sn($max_id);
    }
    else
    {
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('exchange_goods') . " (goods_name, goods_name_style, goods_sn, " .
                "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,sales,spec)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','".$_POST['sales']."','".$_POST['spec']."')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('exchange_goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,sales,spec)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','".$_POST['sales']."','".$_POST['spec']."')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('exchange_goods') . " SET " .
    ...
    if ($is_insert)
    {
        admin_log($_POST['goods_name'], 'add', 'exchange_goods');
    }
    else
    {
        admin_log($_POST['goods_name'], 'edit', 'exchange_goods');
    }
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        $sql = "SELECT attr_id, attr_index FROM " . $ecs->table('exchange_attribute') . " WHERE cat_id = '$goods_type'";
        ...
        $sql = "SELECT g.*, a.attr_type FROM " . $ecs->table('exchange_goods_attr') . " AS g
                LEFT JOIN " . $ecs->table('exchange_attribute') . " AS a ON a.attr_id = g.attr_id
        ...
        $sql = "UPDATE " .$ecs->table('exchange_goods'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('exchange_goods_attr'). " (attr_id, goods_id, attr_value, attr_price)".
                        "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('exchange_goods_attr'). " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
                    $sql = "DELETE FROM " .$ecs->table('exchange_goods_attr'). " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                $db->query($sql);
            }
        }
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('exchange_goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('exchange_goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $ecs->table('exchange_goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('exchange_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
            "VALUES ('$goods_id', '$gallery_img', '', '$gallery_thumb', '$img')";
        $db->query($sql);
    }

    /* 处理相册图片 */
    handle_exchange_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file']);
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $ecs->table('exchange_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
    ...
    {
        $db->query("UPDATE " . $ecs->table('exchange_goods') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
        $db->query("UPDATE " . $ecs->table('exchange_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
        @unlink('../' . $original_img);
        @unlink('../' . $img);
    }
    ...
    /* 提示页面 */
    $link = array();
    if (check_exchange_goods_specifications_exist($goods_id))
    {
        $link[0] = array('href' => 'exchange_goods.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
        $link[1] = array('href' => 'exchange_virtual_card.php?act=replenish&goods_id=' . $goods_id, 'text' => $_LANG['add_replenish']);
    }
    if ($is_insert)
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        {
            /* 检查权限 */
            admin_priv('remove_exchange_back');

            update_exchange_goods($goods_id, 'is_delete', '1');

            /* 记录日志 */
            admin_log('', 'batch_trash', 'exchange_goods');
        }
        /* 上架 */
        elseif ($_POST['type'] == 'on_sale')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_on_sale', '1');
        }
        ...
        elseif ($_POST['type'] == 'not_on_sale')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_on_sale', '0');
        }
        ...
        elseif ($_POST['type'] == 'best')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_best', '1');
        }
        ...
        elseif ($_POST['type'] == 'not_best')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_best', '0');
        }
        ...
        elseif ($_POST['type'] == 'new')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_new', '1');
        }
        ...
        elseif ($_POST['type'] == 'not_new')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_new', '0');
        }
        ...
        elseif ($_POST['type'] == 'hot')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_hot', '1');
        }
        ...
        elseif ($_POST['type'] == 'not_hot')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'is_hot', '0');
        }
        ...
        elseif ($_POST['type'] == 'move_to')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'cat_id', $_POST['target_cat']);
        }
        ...
        elseif ($_POST['type'] == 'suppliers_move_to')
        {
            /* 检查权限 */
            admin_priv('exchange_goods_manage');
            update_exchange_goods($goods_id, 'suppliers_id', $_POST['suppliers_id']);
        }
        ...
        elseif ($_POST['type'] == 'restore')
        {
            ...
            update_exchange_goods($goods_id, 'is_delete', '0');

            /* 记录日志 */
            admin_log('', 'batch_restore', 'exchange_goods');
        }
        /* 删除 */
        elseif ($_POST['type'] == 'drop')
        {
            ...
            delete_exchange_goods($goods_id);

            /* 记录日志 */
            admin_log('', 'batch_remove', 'exchange_goods');
        }
    }
    ...
    {
        $link[] = array('href' => 'exchange_goods.php?act=trash', 'text' => $_LANG['07_exchange_goods_trash']);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'show_image')
{
    ...
    $smarty->display('exchange_goods_show_image.htm');
}
//-- 修改商品名称
elseif ($_REQUEST['act'] == 'edit_goods_name')
{
    check_authz_json('exchange_goods_manage');
...
elseif ($_REQUEST['act'] == 'edit_goods_sn')
{
    ...
    $sql="SELECT goods_id FROM ". $ecs->table('exchange_products')."WHERE product_sn='$goods_sn'";
    if($db->getOne($sql))
    {
        make_json_error($_LANG['goods_sn_exists']);
    }
...
elseif ($_REQUEST['act'] == 'check_products_goods_sn')
{
    ...
    else
    {
        foreach ($products_sn as $val)
        {
            ...
            $sql="SELECT goods_id FROM ". $ecs->table('exchange_products')."WHERE product_sn='$val'";
            if($db->getOne($sql))
            {
                make_json_error($val.$_LANG['goods_sn_exists']);
            }
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
    ...
    // 判断积分商品是否存在货品
    if(check_exchange_goods_product_exist($goods_id) == 1)
    {
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $goods_list = exchange_goods_list($is_delete, ($code=='') ? 1 : 0);
    ...
    $tpl = $is_delete ? 'exchange_goods_trash.htm' : 'exchange_goods_list.htm';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 检查权限 */
    check_authz_json('remove_exchange_back');
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
        ...
        admin_log(addslashes($goods_name), 'trash', 'exchange_goods'); // 记录日志
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
    ...
    check_authz_json('remove_exchange_back'); // 检查权限
    ...
    admin_log(addslashes($goods_name), 'restore', 'exchange_goods'); // 记录日志

    $url = 'exchange_goods.php?act=query&' . str_replace('act=restore_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $ecs->table('exchange_products') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 记录日志 */
    admin_log(addslashes($goods['goods_name']), 'remove', 'exchange_goods');

    /* 删除商品相册 */
    $sql = "SELECT img_url, thumb_url, img_original " .
        "FROM " . $ecs->table('exchange_gallery') .
        " WHERE goods_id = '$goods_id'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    $sql = "DELETE FROM " . $ecs->table('exchange_gallery') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $ecs->table('exchange_goods_attr') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    clear_cache_files();
    $url = 'exchange_goods.php?act=query&' . str_replace('act=drop_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'get_attr')
{
    ...
    $content    = exchange_build_attr_html($goods_type, $goods_id);
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    /* 删除图片文件 */
    $sql = "SELECT img_url, thumb_url, img_original " .
        " FROM " . $GLOBALS['ecs']->table('exchange_gallery') .
        " WHERE img_id = '$img_id'";
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('exchange_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'get_goods_list')
{
    ...
    $arr = get_exchange_goods_list($filters);
    $opt = array();
...
function list_link($is_add = true, $extension_code = '')
{
    ...
    $text = $GLOBALS['_LANG']['03_exchange_goods'];
...
{
    $href = 'exchange_goods.php?act=add';
    $text = $GLOBALS['_LANG']['04_exchange_goods_add'];
    return array('href' => $href, 'text' => $text);
}
