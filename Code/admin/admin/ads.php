 ...
 * 广告管理程序
 * $Author: xy $
 * $Id: ads.php 17217 2014-09-16 xy $
*/
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ads',
        array('ad_link' => $ad_link, 'ad_name' => $ad_name));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if(!preg_match('|^http://|',$ad_link)) {  //如果不能匹配
        $ad_link='http://'.$ad_link;
    }
    if(strlen($ad_link)==7 && $ad_link=='http://') {
        $ad_link = "";
    }
    /*else
    {
    ...
    if ($_POST['media_type'] == '0')
    {
        ...
        {
            $ad_code1 = basename($image->upload_image($_FILES['ad_img'], 'afficheimg'));
            $ad_code = DATA_DIR . '/afficheimg/' .$ad_code1;
        }
    ...
    $link[0]['text'] = $_LANG['continue_add_ad'];
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 0)
    {
        if ((isset($_FILES['ad_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['ad_img']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['ad_img']['tmp_name'] != 'none'))
        {
            ...
            $ad_code1 = DATA_DIR . '/afficheimg/' .$img_up_info;
            $ad_code = "ad_code = '".$ad_code1."'".',';
        }
    ...
    //如果要修改链接图片, 删除原来的图片
    $old_logo = $db->getOne("SELECT ad_code FROM " .$ecs->table('ad'). " WHERE ad_id = '$id'");
    if (!empty($ad_code) || (!empty($_POST['img_url']) && $_POST['img_url'] != $old_logo))
    {
        //获取链子LOGO,并删除
...
function get_adslist()
{
    ...
    $arr = array();
    $sql = 'SELECT ad.* '.
            'FROM ' .$GLOBALS['ecs']->table('ad'). 'AS ad ' .$where .
            'GROUP BY ad.ad_id '.
            'ORDER by '.$filter['sort_by'].' '.$filter['sort_order'];

    $res = $GLOBALS['db']->selectLimit($sql, $filter['page_size'], $filter['start']);
   // $zj_position = array(1=>'定制专区广告',2=>'积分商城广告',3=>'物融广告');
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
       ...
       //  $rows['position_id'] = $zj_position[$rows['position_id']];
         $arr[] = $rows;
    }
