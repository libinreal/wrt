...
require_once(ROOT_PATH . 'includes/lib_exchange_order.php');
require_once(ROOT_PATH . 'includes/lib_exchange_goods.php');
...
{
    /* 检查权限 */
    admin_priv('exchange_order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['05_exchange_order_list']);
    //$smarty->assign('action_link', array('href' => 'exchange_order.php?act=order_query', 'text' => $_LANG['03_exchange_order_query']));
    ...
    $order_list = exchange_order_list();
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('exchange_order_list.htm');
}
//-- 排序、分页、查询
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('exchange_order_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
//-- 订单详情页面
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (isset($_REQUEST['order_id']))
    {
        ...
        $order = exchange_order_info($order_id);
    }
    elseif (isset($_REQUEST['order_sn']))
    {
        $order_sn = trim($_REQUEST['order_sn']);
        $order = exchange_order_info(0, $order_sn);
    }
    else
    {
        /* 如果参数不存在，退出 */
    ...
    if (empty($order))
    {
        die('exchange order does not exist');
    }

    /* 根据订单是否完成检查权限 */
    if (exchange_order_finished($order))
    {
        admin_priv('exchange_order_view_finished');
    }
    else
    {
        admin_priv('exchange_order_view');
    ...
    $sql = "SELECT MAX(order_id) FROM " . $ecs->table('exchange_order_info') . " as o WHERE order_id < '$order[order_id]'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $ecs->table('exchange_order_info') . " as o WHERE order_id > '$order[order_id]'";
    if ($agency_id > 0)
    {
        $sql .= " AND agency_id = '$agency_id'";
    }
    ...
    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
        ...
        "FROM " . $ecs->table('exchange_order_info') . " AS o " .
    ...
    if ($order['order_amount'] < 0)
    {
        ...
        $order['formated_money_refund'] = abs($order['order_amount']);
    }
    ...
    $order['status']        = $_LANG['os'][$order['order_status']] . ',' . $_LANG['ss'][$order['shipping_status']];
    $order['invoice_no']    = $order['shipping_status'] == SS_UNSHIPPED || $order['shipping_status'] == SS_PREPARING ? $_LANG['ss'][SS_UNSHIPPED] : $order['invoice_no'];


    /* 此订单的发货备注(此订单的最后一条操作记录) */
    $sql = "SELECT action_note FROM " . $ecs->table('exchange_order_action').
        " WHERE order_id = '$order[order_id]' AND shipping_status = 1 ORDER BY log_time DESC";
    $order['invoice_note'] = $db->getOne($sql);

    /* 取得订单商品总重量 */
    $weight_price = exchange_order_weight_price($order['order_id']);
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*,  g.goods_number AS storage, o.goods_attr, g.suppliers_id
            FROM " . $ecs->table('exchange_order_goods') . " AS o
                LEFT JOIN " . $ecs->table('exchange_goods') . " AS g
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['formated_goods_price']    = $row['goods_price'];
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['storage'] = '';
            $row['brand_name'] = '';
            // $row['package_goods_list'] = get_package_goods($row['goods_id']); 积分商品不存在礼品
        }
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $ecs->table('exchange_order_action') . " WHERE order_id = '$order[order_id]' ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    /* 取得是否存在实体商品 */
    $smarty->assign('exist_real_goods', exist_real_exchange_goods($order['order_id']));
    ...
    if (isset($_GET['print']))
    {
        ...
        $smarty->display('exchange_order_print.html');
    }
    /* 打印快递单 */
    ...
    {
        /* 模板赋值 */
        $smarty->assign('ur_here', $_LANG['exchange_order_info']);
        $smarty->assign('action_link', array('href' => 'exchange_order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['05_exchange_order_list']));

        /* 显示模板 */
        assign_query_info();
        $smarty->display('exchange_order_info.htm');
    }
}
//-- 发货单列表
...
{
    /* 检查权限 */
    admin_priv('exchange_delivery_view');
    /* 查询 */
    $result = delivery_list();
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['06_exchange_delivery_order']);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('exchange_delivery_list.htm');
}
//-- 搜索、排序、分页
elseif ($_REQUEST['act'] == 'delivery_query')
{
    ...
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('exchange_delivery_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
//-- 发货单详细
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT * FROM " . $ecs->table('exchange_delivery_goods') . "
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $ecs->table('exchange_order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['order_status']    = $_LANG['os'][$row['order_status']];
    ...
    $smarty->assign('action_link', array('href' => 'exchange_order.php?act=delivery_list&' . list_link_postfix(), 'text' => $_LANG['09_delivery_order']));
    $smarty->assign('action_act', ($delivery_order['status'] == 2) ? 'delivery_ship' : 'delivery_cancel_ship');
    assign_query_info();
    $smarty->display('exchange_delivery_info.htm');
    exit; //
}
//-- 发货单发货确认
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    /* 检查此单发货商品库存缺货情况 */
    $virtual_goods = array();
    $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, SUM(DG.send_number) AS sums,  G.goods_number AS storage, G.goods_name, DG.send_number
        FROM " . $GLOBALS['ecs']->table('exchange_delivery_goods') . " AS DG, " . $GLOBALS['ecs']->table('exchange_goods') . " AS G
    ...
    if(!empty($delivery_stock_result))
    {
        foreach ($delivery_stock_result as $value)
        {
            ...
            {
                /* 操作失败 */
                $links[] = array('text' => $_LANG['exchange_order_info'], 'href' => 'exchange_order.php?act=delivery_info&delivery_id=' . $delivery_id);
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        foreach ($delivery_stock_result as $value)
        {
            ...
            if ($value['is_real'] != 0)
            {
                ...
                {
                    $minus_stock_sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_products') . "
                                        SET product_number = product_number - " . $value['sums'] . "
                                        WHERE product_id = " . $value['product_id'];
                    $GLOBALS['db']->query($minus_stock_sql, 'SILENT');
                }

                $minus_stock_sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_goods') . "
    ...
    $query = $db->autoExecute($ecs->table('exchange_delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
    if (!$query)
    {
        /* 操作失败 */
        $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'exchange_order.php?act=delivery_info&delivery_id=' . $delivery_id);
    ...
    update_exchange_order($order_id, $arr);

    /* 发货单发货记录log */
    exchange_order_action($order['order_sn'], OS_CONFIRMED, $shipping_status, $order['pay_status'], $action_note, null, 1);
    ...
    /* 操作成功 */
    $links[] = array('text' => $_LANG['06_exchange_delivery_order'], 'href' => 'exchange_order.php?act=delivery_list');
    $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'exchange_order.php?act=delivery_info&delivery_id=' . $delivery_id);
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    /* 发货单取消发货记录log */
    exchange_order_action($order['order_sn'], $order['order_status'], $shipping_status, $order['pay_status'], $action_note, null, 1);
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        $delivery_stock_sql = "SELECT DG.goods_id, DG.product_id, DG.is_real, SUM(DG.send_number) AS sums".
            " FROM " . $GLOBALS['ecs']->table('exchange_delivery_goods') . " AS DG ".
            " WHERE DG.delivery_id = '$delivery_id' GROUP BY DG.goods_id ";
        $delivery_stock_result = $GLOBALS['db']->getAll($delivery_stock_sql);
...
elseif ($_REQUEST['act'] == 'back_list')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('exchange_back_list.html');
}
//-- 搜索、排序、分页
elseif ($_REQUEST['act'] == 'back_query')
{
    ...
    $sort_flag = sort_flag($result['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('exchange_back_list.html'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT *
                  FROM " . $ecs->table('exchange_back_goods') . "
    ...
    $smarty->assign('action_link', array('href' => 'exchange_order.php?act=back_list&' . list_link_postfix(), 'text' => $_LANG['10_exchange_back_order']));
    assign_query_info();
    $smarty->display('exchange_back_info.html');
    exit; //
}
//-- 修改订单（处理提交）
...
{
    /* 检查权限 */
    admin_priv('exchange_order_edit');
    ...
    if ($order_id > 0)
    {
        $old_order = exchange_order_info($order_id);
    }
    ...
    if ('user' == $step)
    {
        ...
        do
        {
            ...
            if ($db->autoExecute($ecs->table('exchange_order_info'), $order, 'INSERT', '', 'SILENT'))
            {
                break;
            }
            else
        ...
        /* todo 记录日志 */
        admin_log($order['order_sn'], 'add', 'exchange_order');
        ...
        /* 下一步 */
        ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
        exit;
    }
    /* 编辑商品信息 */
    ...
    {//TODO
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
                ...
                {
                    /* 修改 */
                    $sql = "UPDATE " . $ecs->table('exchange_order_goods') .
            ...
            update_exchange_order($order_id, array('goods_amount' => $goods_amount));
    ...
    elseif ('add_goods' == $step)
    {
        ...
        $sql = "SELECT attr_value ".
            'FROM ' . $GLOBALS['ecs']->table('exchange_goods_attr') .
        ...
        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('exchange_products'). " WHERE goods_id = '$goods_id' LIMIT 0, 1";
        ...
        if (is_spec($goods_attr) && !empty($prod))
        {
            if (!empty($goods_attr))
            {
                ...
                {
                    $url = "exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods";
        ...
        {
            /* 插入订单商品 */
            $sql = "INSERT INTO " . $ecs->table('exchange_order_goods') .
        ...
        {
            $sql = "UPDATE " . $ecs->table('exchange_goods') .
        ...
        /* 更新商品总金额和订单总金额 */
        update_exchange_order($order_id, array('goods_amount' => order_amount($order_id)));
        update_order_amount($order_id);
    ...
    elseif ('goods' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
            exit;
        }
        /* 完成 */
        elseif (isset($_POST['finish']))
        {
            ...
            {
                /* 跳转到订单详情 */
                ecs_header("Location: exchange_order.php?act=info&order_id=" . $order_id . "\n");
                exit;
            }
        }
    }
    ...
    elseif ('consignee' == $step)
    {
        ...
        update_exchange_order($order_id, $order);
        ...
        {
            /* 下一步 */
            if (exist_real_exchange_goods($order_id))
            {
                /* 存在实体商品，去配送方式 */
                ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                exit;
            }
        }
        elseif (isset($_POST['finish']))
        {
            ...
            if ('edit' == $step_act && exist_real_exchange_goods($order_id))
            {
                $order = exchange_order_info($order_id);
                ...
                {
                    // 修改配送为空，配送费和保价费为0
                    update_exchange_order($order_id, array('shipping_id' => 0, 'shipping_name' => ''));
                    $links[] = array('text' => $_LANG['step']['shipping'], 'href' => 'exchange_order.php?act=edit&order_id=' . $order_id . '&step=shipping');
            ...
            if ($agency_changed)
            {
                ecs_header("Location: exchange_order.php?act=list\n");
            }
            else
            {
                ecs_header("Location: exchange_order.php?act=info&order_id=" . $order_id . "\n");
    ...
    elseif ('shipping' == $step)
    {
        ...
        /* 取得订单信息 */
        $order_info = exchange_order_info($order_id);
        ...
        $weight_amount = exchange_order_weight_price($order_id);
        ...
        {
            /* 下一步 */
            ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
            ...
            if ('edit' == $step_act && $shipping['support_cod'] == 0)
            {
                ...
                {
                    /* 修改支付为空 */
                    update_exchange_order($order_id, array('pay_id' => 0, 'pay_name' => ''));
                    $msgs[]     = $_LANG['continue_payment'];
                    $links[]    = array('text' => $_LANG['step']['payment'], 'href' => 'exchange_order.php?act=' . $step_act . '&order_id=' . $order_id . '&step=payment');
                }
            }
    ...
    elseif ('payment' == $step)
    {
        ...
        {
            /* 下一步 */
            ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('other' == $step)
    {
        ...
        {
            /* 下一步 */
            ecs_header("Location: exchange_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        /* 取得订单商品 */
        $goods_list = exchange_order_goods($order_id);
        if (!empty($goods_list))
        {
    ...
    {
        /* 查询是否存在实体商品 */
        $exist_real_goods = exist_real_exchange_goods($order_id);
        $smarty->assign('exist_real_goods', $exist_real_goods);
    ...
    elseif ('shipping' == $step)
    {
        ...
        /* 取得配送费用 */
        $total = exchange_order_weight_price($order_id);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('exchange_order_step.htm');
}
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    if ('drop_order_goods' == $func)
    {
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
        {
            $goods = $db->getRow("SELECT goods_id, goods_number FROM " . $ecs->table('exchange_order_goods') . " WHERE rec_id = " . $rec_id );
            $sql = "UPDATE " . $ecs->table('exchange_goods') .
        ...
        /* 删除 */
        $sql = "DELETE FROM " . $ecs->table('exchange_order_goods') .
            " WHERE rec_id = '$rec_id' LIMIT 1";
        $db->query($sql);
    ...
    elseif ('cancel_order' == $func)
    {
        ...
        if ($step_act == 'add')
        {
            ...
            if ($order_id > 0)
            {
                $sql = "DELETE FROM " . $ecs->table('exchange_order_info') .
    ...
    elseif ('refund' == $func)
    {
        ...
        update_exchange_order($order_id, array('order_amount' => 0, 'money_paid' => $order['money_paid'] - $refund_amount));
    ...
    elseif ('load_refund' == $func)
    {
        ...
        assign_query_info();
        $smarty->display('exchange_order_refund.htm');
    }
...
elseif ($_REQUEST['act'] == 'operate')
{
    $order_id = '';
    /* 检查权限 */
    admin_priv('exchange_order_os_edit');
    ...
    {
        /* 检查权限 */
        admin_priv('exchange_order_ps_edit');
    ...
    {
        /* 查询：检查权限 */
        admin_priv('exchange_order_ss_edit');
        ...
        /* 显示模板 */
        $smarty->assign('ur_here', $_LANG['exchange_order_operate'] . $_LANG['op_split']);
        assign_query_info();
        $smarty->display('exchange_order_delivery_info.htm');
        exit;
    }
    /* 未发货 */
    ...
    elseif (isset($_POST['assign']))
    {
        ...
        /* 操作成功 */
        $links[] = array('href' => 'exchange_order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['05_exchange_order_list']);
        sys_msg($_LANG['act_ok'], 0, $links);
    }
    /* 订单删除 */
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$ecs->table('exchange_order_info'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$ecs->table('exchange_order_goods'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$ecs->table('exchange_order_action'). " WHERE order_id = '$order_id'");
            $action_array = array('delivery', 'back');
            del_delivery($order_id, $action_array);

            /* todo 记录日志 */
            admin_log($order['order_sn'], 'remove', 'exchange_order');

            /* 返回 */
            sys_msg($_LANG['order_removed'], 0, array(array('href'=>'exchange_order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])));
        }
    }
    /* 发货单删除 */
    elseif (isset($_REQUEST['remove_invoice']))
    {
        ...
        foreach($delivery_id as $value_is)
        {
            ...
            // 更新：删除发货单
            $sql = "DELETE FROM ".$ecs->table('exchange_delivery_order'). " WHERE delivery_id = '$value_is'";
            $db->query($sql);
        }

        /* 返回 */
        sys_msg($_LANG['tips_delivery_del'], 0, array(array('href'=>'exchange_order.php?act=delivery_list' , 'text' => $_LANG['return_list'])));
    }
    /* 退货单删除 */
    elseif (isset($_REQUEST['remove_back']))
    {
        ...
        if(is_array($back_id))
        {
            ...
            {
                $sql = "DELETE FROM ".$ecs->table('exchange_back_order'). " WHERE back_id = '$value_is'";
                $db->query($sql);
            }
        }
        else
        {
            $sql = "DELETE FROM ".$ecs->table('exchange_back_order'). " WHERE back_id = '$back_id'";
            $db->query($sql);
        }
        /* 返回 */
        sys_msg($_LANG['tips_back_del'], 0, array(array('href'=>'exchange_order.php?act=back_list' , 'text' => $_LANG['return_list'])));
    }
    /* 批量打印订单 */
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            if (exchange_order_finished($order))
            {
                if (!admin_priv('exchange_order_view_finished', '', false))
                {
                    continue;
                }
            }
            ...
            {
                if (!admin_priv('exchage_order_view', '', false))
                {
                    continue;
                }
            }
            ...
            $sql = "SELECT o.*, g.goods_number AS storage, o.goods_attr " .
                "FROM " . $ecs->table('exchange_order_goods') . " AS o ".
                "LEFT JOIN " . $ecs->table('exchange_goods') . " AS g ON o.goods_id = g.goods_id " .
                "WHERE o.order_id = '$order[order_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
            ...
            $smarty->template_dir = '../' . DATA_DIR;
            // 打印发货单
            $html .= $smarty->fetch('exchange_order_print.html') .
    ...
    {
        $url = 'exchange_order.php?act=delivery_list&order_sn='.$_REQUEST['order_sn'];
    ...
    if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund))
    {
        ...
        assign_query_info();
        $smarty->display('exchange_order_operate.htm');
    }
    else
    {
        /* 直接处理 */
        ...
        {
            /* 一个订单 */
            ecs_header("Location: exchange_order.php?act=operate_post&order_id=" . $order_id .
        ...
        {
            /* 多个订单 */
            ecs_header("Location: exchange_order.php?act=batch_operate_post&order_id=" . $order_id .
                "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
            exit;
        }
    }
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    if ('confirm' == $operation)
    {
        ...
        {
            $sql = "SELECT * FROM " . $ecs->table('exchange_order_info') .
            ...
            if($order)
            {
                ...
                /* 标记订单为已确认 */
                update_exchange_order($order_id, array('order_status' => OS_CONFIRMED, 'confirm_time' => gmtime()));
                update_order_amount($order_id);
                /* 记录log */
                exchange_order_action($order['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_UNPAYED, $action_note);
    ...
    elseif ('invalid' == $operation)
    {
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                /* 标记订单为“无效” */
                update_exchange_order($order_id, array('order_status' => OS_INVALID));

                /* 记录log */
                exchange_order_action($order['order_sn'], OS_INVALID, SS_UNSHIPPED, PS_UNPAYED, $action_note);
    ...
    elseif ('cancel' == $operation)
    {
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                update_exchange_order($order_id, array('order_status' => OS_CANCELED, 'to_buyer' => $cancel_note));

                /* 记录log */
                exchange_order_action($order['order_sn'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED, $action_note);
    ...
    elseif ('remove' == $operation)
    {
        ...
        {
            /* 检查能否操作 */
            $order = exchange_order_info('', $id_order);
            $operable_list = operable_list($order);
            if (!isset($operable_list['remove']))
            {
                $sn_not_list[] = $id_order;
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$ecs->table('exchange_order_info'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$ecs->table('exchange_order_goods'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$ecs->table('exchange_order_action'). " WHERE order_id = '$order[order_id]'");
            $action_array = array('delivery', 'back');
...
//    $action_note = $_REQUEST['action_note'];
    ...
    if(empty($sn_not_list))
    {
        ...
        $msg = $sn_list;
        $links[] = array('text' => $_LANG['return_list'], 'href' => 'exchange_order.php?act=list&' . list_link_postfix());
        sys_msg($msg, 0, $links);
    }
    else
    {
        ...
        /* 模板赋值 */
        $smarty->assign('exchange_order_info', $sn_str);
        $smarty->assign('action_link', array('href' => 'exchange_order.php?act=list', 'text' => $_LANG['05_exchange_order_list']));
        $smarty->assign('order_list',   $order_list_no_fail);

        /* 显示模板 */
        assign_query_info();
        $smarty->display('exchange_order_operate_info.htm');
    }
}
//-- 操作订单状态（处理提交）
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED, $action_note);
    }
    /* 设为未付款 */
    ...
    elseif ('prepare' == $operation)
    {
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], OS_CONFIRMED, SS_PREPARING, $order['pay_status'], $action_note);
    ...
    elseif ('split' == $operation)
    {
        ...
        {
            /* 操作失败 */
            $links[] = array('text' => $_LANG['exchange_order_info'], 'href' => 'exchange_order.php?act=info&order_id=' . $order_id);
...
//                }
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            else
            {
                ...
                if (empty($value['product_id']))
                {
                    $sql = "SELECT goods_number FROM " . $GLOBALS['ecs']->table('exchange_goods') . " WHERE goods_id = '" . $value['goods_id'] . "' LIMIT 0,1";
                }
        ...
        $sql ="select add_time from ". $GLOBALS['ecs']->table('exchange_order_info') ." WHERE order_sn = '" . $delivery['order_sn'] . "'";
        ...
        /* 发货单入库 */
        $query = $db->autoExecute($ecs->table('exchange_delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
            ...
            if (!empty($goods_list))
            {
                foreach ($goods_list as $value)
                {
                    ...
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    {
                        ...
                        $query = $db->autoExecute($ecs->table('exchange_delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                    }
                }
            }
        }
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], $arr['order_status'], $shipping_status, $order['pay_status'], $action_note);
    ...
    elseif ('unship' == $operation)
    {
        ...
        update_exchange_order($order_id, array('shipping_status' => SS_UNSHIPPED, 'shipping_time' => 0, 'invoice_no' => '', 'order_status' => OS_CONFIRMED));

        /* 记录log */
        exchange_order_action($order['order_sn'], $order['order_status'], SS_UNSHIPPED, $order['pay_status'], $action_note);
        ...
        /* 将订单的商品发货数量更新为 0 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_order_goods') . "
                SET send_number = 0
    ...
    elseif ('receive' == $operation)
    {
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], $action_note);
    }
    /* 取消 */
    elseif ('cancel' == $operation)
    {
    ...
    elseif ('invalid' == $operation)
    {
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], OS_INVALID, $order['shipping_status'], PS_UNPAYED, $action_note);
    ...
    elseif ('return' == $operation)
    {
        ...
        /* 记录log */
        exchange_order_action($order['order_sn'], OS_RETURNED, SS_UNSHIPPED, PS_UNPAYED, $action_note);
        ...
        $delivery_list = array();
        $sql_delivery = "SELECT *
                         FROM " . $ecs->table('exchange_delivery_order') . "
                         WHERE status IN (0, 2)
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back_goods = "INSERT INTO " . $ecs->table('exchange_back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
                                   SELECT '$back_id', goods_id, product_id, product_sn, goods_name, goods_sn, is_real, send_number, goods_attr
                                   FROM " . $ecs->table('exchange_delivery_goods') . "
        ...
        /* 修改订单的发货单状态为退货 */
        $sql_delivery = "UPDATE " . $ecs->table('exchange_delivery_order') . "
                         SET status = 1
                         WHERE status IN (0, 2)
                         AND order_id = " . $order['order_id'];
    ...
    {
        /* 记录log */
        exchange_order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], '[' . $_LANG['op_after_service'] . '] ' . $action_note);
    }
    else
    {
        die('invalid params');
...
elseif ($_REQUEST['act'] == 'json')
{
    ...
    if ($func == 'get_goods_info')
    {
        ...
        $sql = "SELECT goods_id, c.cat_name, goods_sn, goods_name, " .
            ...
            "FROM " . $ecs->table('exchange_goods') . " AS g " .
            "LEFT JOIN " . $ecs->table('exchange_category') . " AS c ON g.cat_id = c.cat_id " .
        ...
        $sql = "SELECT a.attr_id, a.attr_name, g.goods_attr_id, g.attr_value, g.attr_price, a.attr_input_type, a.attr_type " .
            "FROM " . $ecs->table('exchange_goods_attr') . " AS g, " .
            $ecs->table('exchange_attribute') . " AS a " .
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['ecs']->table('exchange_goods') .
            " WHERE is_delete = 0" .
            " AND is_on_sale = 1" .
            " AND is_alone_sale = 1" .
...
elseif ($_REQUEST['act'] == 'edit_invoice_no')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('exchange_order_info') . " SET invoice_no='$no' WHERE order_id = '$order_id'";
...
elseif ($_REQUEST['act'] == 'edit_pay_note')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('exchange_order_info') . " SET pay_note='$no' WHERE order_id = '$order_id'";
    if ($GLOBALS['db']->query($sql))
    {
        if (empty($no))
        {
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    $smarty->assign('goods_attr', $attr);
    $smarty->assign('goods_list', $goods_list);
    $str = $smarty->fetch('exchange_order_goods_info.htm');
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('exchange_order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
    }
    ...
    update_exchange_order($order['order_id'], $arr);
}
...
{
    include_once(ROOT_PATH . 'includes/lib_exchange_order.php');
    //更新订单总金额
    $sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_order_info') .
...
function handle_order_money_change($order, &$msgs, &$links)
{
    ...
    if ($order['pay_status'] == PS_PAYED || $order['pay_status'] == PS_PAYING)
    {
        ...
        {
            /* 修改订单为未付款 */
            update_exchange_order($order_id, array('pay_status' => PS_UNPAYED, 'pay_time' => 0));
            $msgs[]     = $GLOBALS['_LANG']['amount_increase'];
            $links[]    = array('text' => $GLOBALS['_LANG']['exchange_order_info'], 'href' => 'exchange_order.php?act=info&order_id=' . $order_id);
        }
        elseif ($money_dues < 0)
        {
            $anonymous  = $order['user_id'] > 0 ? 0 : 1;
            $msgs[]     = $GLOBALS['_LANG']['amount_decrease'];
            $links[]    = array('text' => $GLOBALS['_LANG']['refund'], 'href' => 'exchange_order.php?act=process&func=load_refund&anonymous=' .
...
function exchange_order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('exchange_order_info') . " AS o ,".
                $GLOBALS['ecs']->table('users') . " AS u " . $where;
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('exchange_order_info') . " AS o ". $where;
        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel,  " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
            "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
            " FROM " . $GLOBALS['ecs']->table('exchange_order_info') . " AS o " .
...
function update_pay_log($order_id)
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT order_amount FROM " . $GLOBALS['ecs']->table('exchange_order_info') .
            " WHERE order_id = '$order_id'";
        ...
        if (!is_null($order_amount))
        {
            $sql = "SELECT log_id FROM " . $GLOBALS['ecs']->table('pay_log') .
                ...
                " AND is_paid = 0 AND is_exchange=1";
            ...
            else
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('pay_log') .
                    " (order_id, order_amount, order_type, is_paid,is_exchange)" .
                    "VALUES('$order_id', '$order_amount', '" . PAY_ORDER . "', 0,1)";
            }
...
{
    $goods_list = array();
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id, g.goods_number AS storage " .
        "FROM " . $GLOBALS['ecs']->table('exchange_order_goods') . " AS o ".
        "LEFT JOIN " . $GLOBALS['ecs']->table('exchange_goods') . " AS g ON o.goods_id = g.goods_id " .
        "WHERE o.order_id = '$order[order_id]' ";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
    ...
    return array('goods_list' => $goods_list);
}
...
function order_deliveryed($order_id)
{
    ...
    $sql = "SELECT COUNT(delivery_id) FROM ". $GLOBALS['ecs']->table('delivery_order') ." WHERE order_id = ". $order_id ." AND status = 0";
...
function get_order_finish($order_id)
{
    ...
    $sql = 'SELECT COUNT(rec_id)
            FROM ' . $GLOBALS['ecs']->table('exchange_order_goods') . '
            WHERE order_id = \'' . $order_id . '\'
...
function get_all_delivery_finish($order_id)
{
    ...
    else
    {
        ...
        $sql = "SELECT COUNT(delivery_id)
                FROM " . $GLOBALS['ecs']->table('exchange_delivery_order') . "
                WHERE order_id = '$order_id'
                AND status = 2 ";
        $sum = $GLOBALS['db']->getOne($sql);
        // 全部发货
...
function del_order_delivery($order_id)
{
    ...
    $sql = 'DELETE O, G
            FROM ' . $GLOBALS['ecs']->table('exchange_delivery_order') . ' AS O, ' . $GLOBALS['ecs']->table('exchange_delivery_goods') . ' AS G
            WHERE O.order_id = \'' . $order_id . '\'
            AND O.status = 0
...
function del_delivery($order_id, $action_array)
{
    ...
    {
        $sql = 'DELETE O, G
                FROM ' . $GLOBALS['ecs']->table('exchange_back_order') . ' AS O, ' . $GLOBALS['ecs']->table('exchange_back_goods') . ' AS G
                WHERE O.order_id = \'' . $order_id . '\'
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('exchange_delivery_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('exchange_back_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT back_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                ...
                FROM " . $GLOBALS['ecs']->table("exchange_back_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('exchange_delivery_order');
    if ($delivery_id > 0)
    {
...
function back_order_info($back_id)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('exchange_back_order') . "
...
function package_sended($package_id, $goods_id, $order_id, $extension_code, $product_id = 0)
{
    ...
    $sql = "SELECT SUM(DG.send_number) FROM " . $GLOBALS['ecs']->table('exchange_delivery_goods') . " AS DG,
            " . $GLOBALS['ecs']->table('exchange_delivery_order') . " AS o
        ...
        if (is_array($value))
        {
            ...
            foreach ($goods_list as $goods)
            {
                ...
                foreach ($goods['package_goods_list'] as $package_goods)
                {
                    ...
                    $sql = "UPDATE " . $GLOBALS['ecs']->table('exchange_goods') ."
...
function package_virtual_card_shipping($goods, $order_sn)
{
    ...
    foreach ($goods as $virtual_goods_key => $virtual_goods_value)
    {
        ...
        // TODO
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email FROM ".$GLOBALS['ecs']->table('exchange_order_info'). " WHERE order_sn = '$order_sn'";
...
function delivery_return_goods($delivery_id, $delivery_order)
{
    /* 查询：取得发货单商品 */
    $goods_sql = "SELECT *
                 FROM " . $GLOBALS['ecs']->table('exchange_delivery_goods') . "
                 WHERE delivery_id = " . $delivery_order['delivery_id'];
    $goods_list = $GLOBALS['db']->getAll($goods_sql);
    /* 更新： */
...
function del_order_invoice_no($order_id, $delivery_invoice_no)
{
    ...
    $sql = "SELECT invoice_no
            FROM " . $GLOBALS['ecs']->table('exchange_order_info') . "
            WHERE order_id = '$order_id'";
...
{
    return 'http://' . $_SERVER['HTTP_HOST'] . str_replace('/' . ADMIN_PATH . '/exchange_order.php', '', PHP_SELF);
