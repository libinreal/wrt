 ...
 * 物融新闻分类管理程序
 * $Author: xy $
 * $Id: wrnewscat.php 2014-09-04 xy $
*/
...
{
    $catid = $db->getOne("SELECT cat_id FROM article_cat WHERE cat_type = " . 1000);
    ...
    $smarty->assign('ur_here',     $_LANG['02_wrnewscat_list']);
    //$smarty->assign('action_link', array('text' => $_LANG['wrnewscat_add'], 'href' => 'wrnewscat.php?act=add'));
    $smarty->assign('full_page',   1);
    $smarty->assign('articlecat',        $articlecat);

    assign_query_info();
    $smarty->display('wrnewscat_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('wrnewscat_list.htm'));
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',     $_LANG['wrnewscat_add']);
    $smarty->assign('action_link', array('text' => $_LANG['02_wrnewscat_list'], 'href' => 'wrnewscat.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('wrnewscat_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    admin_log($_POST['cat_name'],'add','wrnewscat');

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'wrnewscat.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'wrnewscat.php?act=list';
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', updateAt='$updateat', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        $link[0]['href'] = 'wrnewscat.php?act=list';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'wrnewscat.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
