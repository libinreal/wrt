...
if($_REQUEST['act'] == 'list_arr') {
    ...
    assign_query_info();
    $smarty->display('order_list_arr.htm');
}
...
// 催单列表
elseif ($_REQUEST['act'] == 'reminder') {
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['11_order_reminder']);
    $smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['02_order_list']));

    $smarty->assign('status_list', $_LANG['cs']);   // 订单状态
    $smarty -> assign('myStatus',-1);
    $smarty->assign('full_page',        1);
    $order_reminder  = get_order_reminder();
    $smarty->assign('order_list',   $order_reminder['orders']);
    $smarty->assign('filter',       $order_reminder['filter']);
    $smarty->assign('record_count', $order_reminder['record_count']);
    $smarty->assign('page_count',   $order_reminder['page_count']);
    $smarty->assign('sort_order_time', '<img src="images/sort_desc.gif">');
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_reminder.html');
}
// 订单备注
elseif ($_REQUEST['act'] == 'remarks') {
    admin_priv('order_view');
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_remarks.html');
}
elseif ($_REQUEST['act'] == 'remarks_save') {
    admin_priv('order_view');
    if (isset($_REQUEST['order_id']))
    {
        $order_id = intval($_REQUEST['order_id']);
    ...
    $postscript = strip_tags($_REQUEST['postscript']);

    $sql = "UPDATE ".$GLOBALS['ecs']->table('order_info')." SET postscript='".$postscript."' WHERE order_id=".$order_id."";
    if($GLOBALS['db']->query($sql)) {

        $links[0] = array ('text' => $_LANG['02_order_list'],'href' => 'order.php?act=list');
        clear_cache_files(); // 清除缓存文件
        sys_msg($_LANG['remarks_save_ok'], 0, $links);
    }
}
//-- 订单详情页面
elseif ($_REQUEST['act'] == 'info')
{
    ...
    //echo $order['order_parent_id'];
    if($order['parent_order_id']!=0) {
        $order['status']        = $_LANG['cos'][$order['order_status']];
    }else {
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*,  g.goods_number AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name
...
// 查看子订单
elseif ($_REQUEST['act'] == 'suborders') {
    admin_priv('order_view');
    $order_id = intval($_GET['order_id']);

    $filter = array();
    $smarty -> assign('ur_here', $_LANG['suborders_list']);
    $smarty -> assign('action_link', array ('text' => $_LANG['02_order_list'] ,'href' => 'order.php?act=list' ));
    $smarty -> assign('full_page',1);
    $smarty -> assign('filter', $filter);

    $subOrders = get_suborders($order_id);
    $smarty -> assign('suborders',$subOrders);
    assign_query_info();
    $smarty->display('order_suborders.html');
}
//订单拆分
elseif ($_REQUEST['act'] == 'split') {
    ...
    $smarty->assign('ur_here',      $_LANG['order_split']);
    $smarty->assign('action_link',  array('text' => $_LANG['02_order_list'], 'href' => 'order.php?act=list'));
    // 订单状态
    //$order['status'] = $_LANG['os'][$order['order_status']] . ',' . $_LANG['ps'][$order['pay_status']] . ',' . $_LANG['ss'][$order['shipping_status']];
    $order['status'] = $_LANG['os'][$order['order_status']];
    // 获取订单的商品信息
    $sql = "SELECT o.*,  g.goods_number AS storage,g.suppliers_id,g.wcode,s.suppliers_name,c.measure_unit, IFNULL(b.brand_name, '') AS brand_name
            FROM " . $ecs->table('order_goods') . " AS o
            ...
            LEFT JOIN ". $GLOBALS['ecs']->table('category')." as c ON c.code=g.cat_code
            WHERE o.order_id = '$order[order_id]'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['formated_subtotal']       = price_format($row['goods_price'] * $row['goods_number']);
        $row['formated_goods_price']    = price_format($row['goods_price']);
        $row['no_split'] = $row['goods_number'] - $row['send_number'];
        $goods_list[] = $row;
    }
    $smarty -> assign("goods_list",$goods_list);
    $smarty -> assign("order",$order);
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_split.htm');
}
// 拆分订单列表
elseif($_REQUEST['act'] == 'goods_split') {
    ...
    if(!$order_id || !$goods_id) {
        ...
        $sql_order = "SELECT * FROM ". $GLOBALS['ecs'] -> table('order_info') ." WHERE order_id=".$order_id."";
        $order_row = $GLOBALS['db'] -> getRow($sql_order);
        $sql_goods = "SELECT o.*,c.measure_unit,s.suppliers_name,g.wcode FROM ". $GLOBALS['ecs'] -> table('order_goods')." AS o ".
            " LEFT JOIN ". $GLOBALS['ecs']->table('goods')." AS g ON o.goods_id=g.goods_id ".
            " LEFT JOIN ". $GLOBALS['ecs']->table('category')." AS c ON g.cat_code=c.code ".
            " LEFT JOIN ".$GLOBALS['ecs']->table('suppliers')." AS s ON s.suppliers_id=g.suppliers_id ".
            " WHERE o.goods_id=".$goods_id." AND o.order_id=".$order_id." ";
        $goods = $GLOBALS['db'] -> getRow($sql_goods);
        $goods['formated_goods_price'] = price_format($goods['goods_price']);
        $goods['no_send_number'] = $goods['goods_number'] - $goods['send_number'];
    }
    $sql_suppliers = "select suppliers_id,suppliers_name from ". $GLOBALS['ecs'] -> table('suppliers') ." ";
    $suppliers = $GLOBALS['db'] -> getAll($sql_suppliers);
    $sql = "select order_sn from ".$GLOBALS['ecs']->table('order_info')." where parent_order_sn ='".$order_row['order_sn']."' order by order_id desc limit 1";
    $childer_order_sn = $GLOBALS['db']->getOne($sql);
    if(!$childer_order_sn) {
        $childer_order_sn = $order_row['order_sn']."-1";
    }else {
        $childer = explode('-',$childer_order_sn);
        $childer[1] = $childer[1] +1;
        $childer_order_sn = $childer[0].'-'.$childer[1];
    }
    if(empty($order_row['best_time'])) {
        $dataTime = date("Y-m-d",time()+86400*3);
    }else {
        $dataTime = date('Y-m-d',$order_row['best_time']);
    }
    $smarty->assign('ur_here',      $_LANG['order_split']);
    $smarty->assign('action_link',  array('text' => $_LANG['02_order_list'], 'href' => 'order.php?act=list'));
    $smarty -> assign('childer_order_sn',$childer_order_sn);
    $smarty -> assign('order',$order_row);
    $smarty -> assign('goods',$goods);
    $smarty -> assign('suppliers',$suppliers);
    $smarty -> assign('dataTime',$dataTime);

    assign_query_info();
    $smarty->display('order_goods_split.htm');
}
// 保存拆分的订单
elseif($_REQUEST['act'] == 'split_save') {
    check_authz_json('order_edit');
    // 订单编号
    if(isset($_REQUEST['order_sn'])) {
        foreach($_REQUEST['order_sn'] as $key=>$value) {
            $order[$key]['order_sn'] = $_REQUEST['order_sn'][$key];
        }
    }
    // 供货商ID
    if(isset($_REQUEST['suppers_id'])) {
        foreach($_REQUEST['suppers_id'] as $key=>$value) {
            $order[$key]['suppers_id'] = $_REQUEST['suppers_id'][$key];
        }
    }
    // 商品价格
    if(isset($_REQUEST['goods_price'])) {
        foreach($_REQUEST['goods_price'] as $key=>$value) {
            $order[$key]['goods_price'] = $value;
        }
    }
    // 发货数量
    if(isset($_REQUEST['send_number'])) {
        foreach($_REQUEST['send_number'] as $key=>$value) {
            $order[$key]['send_number'] = $value;
        }
    }
    // 最佳配送时间
    if(isset($_REQUEST['best_time'])) {
        foreach($_REQUEST['best_time'] as $key=>$value) {
            $order[$key]['best_time'] = strtotime($value);
        }
    }
    // 订单id 上级订单的
    if(isset($_REQUEST['order_id'])) {
    ...
    // 上级订单编号
    if(isset($_REQUEST['parent_order_sn'])) {
        $parent_order_sn =$_REQUEST['parent_order_sn'];
    }

    // 查询上级订单的信息 将收货人信息等 复制给子订单
    $sql ="SELECT * FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=".$order_id." AND order_sn=".$parent_order_sn."";
    $row = $GLOBALS['db']-> getRow($sql);
    $row['parent_order_id'] = $order_id;
    $row['parent_order_sn'] = $row['order_sn'];
    $row['confirms_time'] = gmtime();
    unset($row['best_time']);
    unset($row['order_sn']);
    unset($row['send_number']);
    unset($row['suppers_id']);

    foreach($order as $k=>$v) {
        $arr[$k]['order'] = array_merge($v,$row);
    }
    $sqlgoods ="SELECT * FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id=".$order_id." AND goods_id=".$goods_id."";
    $rowGoods = $GLOBALS['db']->getRow($sqlgoods);
    unset($rowGoods['send_number']);
    foreach($order as $k=>$val) {
        $arr[$k]['goods'] = array_merge($val,$rowGoods);
    }

    foreach($arr as $k=>$info) {
        $infoBak = $info;
        // 判断可以发货的数量 在进行生成订单流水 如果订单数量 - 已发货数量 大于等于拆分订单的数量 则进行拆分 否则不拆分
        $sqlOrderGoods = "SELECT goods_number,send_number from ".$GLOBALS['ecs']->table('order_goods')."
                            WHERE goods_id=".$goods_id." AND order_id=".$order_id."";
        $rowGoodsNum = $GLOBALS['db']->getRow($sqlOrderGoods);

        if(($rowGoodsNum['goods_number']-$rowGoodsNum['send_number'])>= $info['order']['send_number']) {
            //unset($info['order']['suppers_id']);
            unset($info['order']['order_id']);
            $info['order']['goods_amount'] = ((double)$info['order']['send_number'] * (double)$info['order']['goods_price']);
            $info['order']['order_amount'] = ((double)$info['order']['send_number'] * (double)$info['order']['goods_price']);
            $info['order']['add_time'] = gmtime();
            $info['order']['order_status'] = OS_CHILD_ORDER;
            $info['order']['shipping_status'] = 0;
            $info['order']['shipping_time'] = gmtime();
            unset($info['order']['send_number']);
            unset($info['order']['goods_price']);
            foreach($info['order'] as $key=>$val) {
                $arr1 .= ','.$key;
                $arr2 .= ",'".$val."'";
            }
            $arrfiledOrder = substr($arr1,1);
            $arrVaueOrder = substr($arr2,1);
            unset($arr1);
            unset($arr2);
            $action_note = "订单拆分";
            // 插入订单
            $sql_order_info_insert ="INSERT INTO ".$GLOBALS['ecs']->table('order_info')."($arrfiledOrder) VALUE($arrVaueOrder)";
            $GLOBALS['db']->query($sql_order_info_insert);
            $insert_order_id = $GLOBALS['db'] -> insert_id(); // 获取新插入子订单的订单id
            $customNo = $GLOBALS['db']->getRow("SELECT u.customNo,oi.order_sn as order_sn FROM ".$GLOBALS['ecs']->table('users')." AS u INNER JOIN ".$GLOBALS['ecs']->table('order_info')." AS oi ON u.user_id=oi.user_id WHERE oi.order_id=".$insert_order_id."");
            order_action($info['order']['order_sn'], OS_SPLITED, SS_SHIPPED_ING, PS_UNPAYED, $action_note);
            admin_log($info['order']['order_sn'], 'split_save', 'order_info');

            unset($info['goods']['order_sn']);
            unset($info['goods']['rec_id']);
            unset($info['goods']['best_time']);
            unset($info['goods']['suppers_id']);
            $info['goods']['goods_number'] = $info['goods']['send_number'];
            $info['goods']['order_id'] = $insert_order_id;
            foreach($info['goods'] as $key=>$val) {
                $arrkey .= ','.$key.'';
                $arrVal .= ",'".$val."'";
            }
            $arrfiled = substr($arrkey,1);
            $arrVaue = substr($arrVal,1);
            unset($arrkey);
            unset($arrVal);
            $sql_order_goods_insert ="INSERT INTO ".$GLOBALS['ecs']->table('order_goods')."($arrfiled) VALUE($arrVaue)";
            $GLOBALS['db']->query($sql_order_goods_insert);

            $insert_order_goods_id = $GLOBALS['db'] -> insert_id();
            $sql_send_nember = "UPDATE ".$GLOBALS['ecs']->table('order_goods')." SET send_number=(send_number+".$info['goods']['send_number'].") ".
                "WHERE goods_id='".$goods_id."' AND order_id='".$order_id."'";
            $GLOBALS['db']->query($sql_send_nember);

            if($info['order']['order_status']==-1) $info['order']['order_status'] = 'A';

            $og_wcode = $GLOBALS['db'] -> getOne("SELECT g.wcode FROM ".$GLOBALS['ecs']->table('goods')." AS g INNER JOIN ".$GLOBALS['ecs']->table('order_goods')." as og ON g.goods_id=og.goods_id WHERE og.goods_id=".$info['goods']['goods_id']."");
            $suppliers_code = $GLOBALS['db'] -> getOne("SELECT suppliers_code from ".$GLOBALS['ecs']->table('suppliers')." WHERE suppliers_id=".$info['order']['suppers_id']."");
            $measure_unit = $GLOBALS['db'] -> getOne("select  c.measure_unit from ".$GLOBALS['ecs']->table('goods')." AS g INNER JOIN ".$GLOBALS['ecs']->table('category')." AS c ON g.cat_id=c.cat_id WHERE g.goods_id=".$info['goods']['goods_id']."");
            $brand_code = $GLOBALS['db'] -> getOne("SELECT b.brand_code FROM ".$GLOBALS['ecs']->table('goods')." AS g INNER JOIN ".$GLOBALS['ecs']->table('brand')." AS b ON g.brand_id=b.brand_id WHERE g.goods_id=".$info['goods']['goods_id']."");
            $fall_count = ($info['goods']['goods_number'] * $info['goods']['goods_price']);
            $orderArr = array(
                'cusFnum' => $customNo['customNo'],//客户编号
                'b2bFnum' => $customNo['order_sn'],//商城订单号"
                'erpFnum' => $info['order']['erp_sn'],//erp订单编号
                'conFnum' => $info['order']['contract_sn'],//对应合同编号
                'erpState' => $info['order']['order_status'],//订单状态（A保存,B提交,C审核）
                'saleDate' => date('Y-m-d',$info['order']['add_time']),//订单日期yyyy-MM-dd
                'receiveFnum' => $customNo['customNo'],//收货方编码
                'chargeFnum' => $customNo['customNo'],//收款方编码
                'settleFnum' => $customNo['customNo'],//结算方编码
                'remark' => $info['order']['postscript'],//备注
                'receiveAddress' => $info['order']['address'],//地址
                'supplyNumber' => $suppliers_code,//供应商
                'fdeliverydate' => date('Y-m-d',$info['order']['best_time']),//要货日期yyyy-MM-dd
                'SAL_SaleOrder__FSaleOrderEntry' => array(array(
                    'matFnum' => $og_wcode,//物料编码
                    'qtyOrder' => $info['goods']['goods_number'],//订单数量
                    'priceOrder' => $info['goods']['goods_price'],//"//订单含税单价
                    'qtyAudit' => $info['goods']['contract_number'],//": 100,//审核数量
                    'priceAudit' => $info['goods']['contract_price'],//": 10,审核单价
                    'qtyExam' => $info['goods']['check_number'],//": 100,//验货数量
                    'priceExam' => $info['goods']['check_price'],//验货单价
                    'baseUnitFnum' => $measure_unit,//基本计量单位
                    'FTAXRATE' => '',//税率
                    'FbrandNum' => $brand_code,//010001",$brand_code
                    'FALLAMOUNT' => $fall_count,//价税合计
                )),
            );
            //$url = "http://61.155.169.177:9988/api/saveOrderAction?orderstr=".json_encode($orderArr);
            $url = $ecs->url()."api/saveOrder";
            $orderStrInfo = array('orderstr'=>json_encode($orderArr));
            $ordererpsuccess = get_orderInfo($url,$orderStrInfo);
            //$ordererpsuccess = json_decode($ordererpsuccess, true);
            //if (is_array($ordererpsuccess) && $ordererpsuccess['isSuccess'] === 0) {
            //    $flag = $GLOBALS['db'] -> query("UPDATE order_info SET order_status='".OS_UNCONFIRMED."' WHERE order_id=".$insert_order_id."");
            //}
            //$sql_order_info_status = "UPDATE ".$GLOBALS['ecs']->table('order_info')." SET order_status='".OS_SPLITING_PART."'".
            //    "  WHERE order_id=".$order_id." AND order_sn='".$info['order']['parent_order_sn']."'";
            //$GLOBALS['db']->query($sql_order_info_status);
        } else {
          // 拆分失败
        }
    }
    admin_log($order['order_sn'], 'split', 'order');
    $link[] = array('href' => 'order.php?act=list', 'text' => $_LANG['02_order_list']);
    clear_cache_files();
...
{
    $order_id = $_REQUEST["order_id"];
    $sql1 = "SELECT oi.*,og.* FROM ".$GLOBALS['ecs']->table('order_info')." AS oi INNER JOIN ".$GLOBALS['ecs']->table('order_goods')." as og ON oi.order_id=og.order_id WHERE oi.order_id=".$order_id."";
    $info = $GLOBALS['db']->getRow($sql1);
    $sql2 ="SELECT * FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id=".$info['user_id']."";
    $customNo = $GLOBALS['db']->getRow($sql2);

    $fall_count = ($info['goods_number'] * $info['goods_price']);

    $sql3 = " SELECT g.wcode,sp.suppliers_code,b.brand_code,cate.measure_unit FROM goods AS g ".
            " INNER JOIN suppliers AS sp ON g.suppliers_id=sp.suppliers_id ".
            " LEFT JOIN brand AS b ON g.brand_id=b.brand_id ".
            " LEFT JOIN category AS cate ON g.cat_id=cate.cat_id ".
            " WHERE g.goods_id=".$info['goods_id']."";
    $orderCode = $GLOBALS["db"]->getRow($sql3);
    $orderArr = array(
        'cusFnum' => $customNo['customNo'],//客户编号
        'b2bFnum' => $info['order_sn'],//商城订单号"
        'erpFnum' => $info['erp_sn'],//erp订单编号
        'conFnum' => $info['contract_sn'],//对应合同编号
        'erpState' => 'A',//订单状态（A保存,B提交,C审核）//TODO 20150922
        'saleDate' => date('Y-m-d',$info['add_time']),//订单日期yyyy-MM-dd
        'receiveFnum' => $customNo['customNo'],//收货方编码
        'chargeFnum' => $customNo['customNo'],//收款方编码
        'settleFnum' => $customNo['customNo'],//结算方编码
        'remark' => $info['postscript'],//备注
        'receiveAddress' => $info['address'],//地址
        'supplyNumber' => $orderCode['suppliers_code'],//供应商
        'fdeliverydate' => date('Y-m-d',$info['best_time']),//要货日期yyyy-MM-dd
        'SAL_SaleOrder__FSaleOrderEntry' => array(array(
            'matFnum' => $orderCode['wcode'],//物料编码
            'qtyOrder' => $info['goods_number'],//订单数量
            'priceOrder' => $info['goods_price'],//"//订单含税单价
            'qtyAudit' => $info['contract_number'],//": 100,//审核数量
            'priceAudit' => $info['contract_price'],//": 10,审核单价
            'qtyExam' => $info['check_number'],//": 100,//验货数量
            'priceExam' => $info['check_price'],//验货单价
            'baseUnitFnum' => $orderCode['measure_unit'],//基本计量单位
            'FTAXRATE' => '',//税率
            'FbrandNum' => $orderCode['brand_code'],//010001",$brand_code
            'FALLAMOUNT' => $fall_count,//价税合计
        )),
    );
    //$url = "http://61.155.169.177:9988/api/saveOrderAction?orderstr=".json_encode($orderArr);
    ...
    $link[] = array('href' => 'order.php?act=list', 'text' => $_LANG['02_order_list']);
    clear_cache_files();
    $rs = json_decode($ordererpsuccess, 1);
    if($rs['code'] == 0) {
	    sys_msg($_LANG['batch_handle_ok'], 0);
    } else {
    	sys_msg($rs['message'], 1);
    }
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT * FROM " . $ecs->table('delivery_goods') . "
...
function operable_list($order)
{
    ...
    if ($actions == 'all')
    {
        $priv_list  = array('os' => true, 'edit' => true);
    }
    else
    {
        $actions    = ',' . $actions . ',';
...
function get_order_reminder() {
    ...
    if ($result === false)
    {
        ...
        $filter['order_amount'] = empty($_REQUEST['order_amount']) ? '' : trim($_REQUEST['order_amount']);
        $filter['order_status'] = isset($_REQUEST['order_status']) ? intval($_REQUEST['order_status']) : -1;
        $filter['contract_sn'] =isset($_REQUEST['contract_sn']) ? trim($_REQUEST['contract_sn']) : '' ;
        $filter['consignee'] = empty($_REQUEST['consignee']) ? '' : trim($_REQUEST['consignee']);
        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'add_time' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

        $where = 'WHERE 1=1 AND o.parent_order_id=0 and is_remaind=1 ';
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.contract_sn,o.parent_order_id, " .
            "o.parent_order_sn, (" . order_amount_field('o.') . ") AS total_fee, " .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['formated_total_fee'] =  price_format($value['order_amount']);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = 'WHERE 1=1 AND o.parent_order_id=0  ';
        if ($filter['order_sn'])
        {
        ...
        {
            $where .= " AND o.contract_sn like '%".$filter['contract_sn']."%' ";
        }
        // 订单状态
        if ($filter['composite_status'] != -1) {
            $where .= " AND o.order_status = '$filter[composite_status]' ";
 ...
 * @retrun array
 */
function get_suborders($order_id) {

    $sql = " SELECT o.*,IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
            " FROM ".$GLOBALS['ecs']->table('order_info')." AS o ".
            " INNER JOIN ".$GLOBALS['ecs']->table('users')." AS u ON o.user_id=u.user_id ".
            " WHERE o.parent_order_id='".$order_id."' ORDER BY order_sn ASC ";
    $result = $GLOBALS['db'] -> getAll($sql);
    foreach($result as $key=>$row) {
        $result[$key]['short_order_time'] = local_date('m-d H:i', $value['add_time']);
        $result[$key]['formated_total_fee'] = price_format($row['order_amount']);
    }
    return $result;
}
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT delivery_id, delivery_sn, parent_order_sn,order_sn, order_id, add_time, action_user, consignee, country,
...
function get_orderInfo($url, $content) {
