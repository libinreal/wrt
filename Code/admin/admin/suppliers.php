...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('suppliers_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
//-- 列表页编辑名称
...
elseif ($_REQUEST['act'] == 'is_check')
{
    check_authz_json('suppliers_manage');
    $id = intval($_REQUEST['id']);
    $sql = "SELECT suppliers_id, is_check FROM " . $ecs->table('suppliers') . "
            WHERE suppliers_id = '$id'";
    $suppliers = $db->getRow($sql, TRUE);
    if ($suppliers['suppliers_id'])
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            $sql = "SELECT *  FROM " . $ecs->table('suppliers') . "
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
    ...
    if ($_REQUEST['act'] == 'add')
    {
        ...
        $smarty -> assign('arealist',get_regions(1,1));
        assign_query_info();
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        if(!isset($_POST['admins'])) {
            sys_msg($_LANG['suppliers_admins_exist']);
        }
