 ...
 */
function exchange_order_info($order_id, $order_sn = '')
{
    /* 计算订单各种费用之和的语句 */
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['ecs']->table('exchange_order_info') .
 ...
 * @return  bool
 */
function exchange_order_finished($order)
{
 ...
 * @return  array   订单商品数组
 */
function exchange_order_goods($order_id)
{
 ...
 */
function exchange_order_weight_price($order_id)
{
    $sql = "SELECT SUM(g.goods_weight * o.goods_number) AS weight, " .
        ...
        $GLOBALS['ecs']->table('exchange_goods') . " AS g " .
 ...
 * @return  bool
 */
function update_exchange_order($order_id, $order)
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('exchange_order_info'),
 ...
 * @return  bool
 */
function exist_real_exchange_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{

    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('exchange_order_goods') . " WHERE order_id = '$order_id' AND is_real = 1";
    return $GLOBALS['db']->getOne($sql) > 0;
}
...
{
    if (exist_real_exchange_goods(0, $flow_type))
    {
        /* 如果存在实体商品 */
