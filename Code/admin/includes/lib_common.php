...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children,c.code,c.wcode,c.cat_level ".
    ...
    $children_level = 999999; //大于这个分类的将被删除
    ...
    if ($re_type == true)
    {
        ...
        {
            $select .= '<option value="' . $var['cat_id'] . '" data-code="'.$var['code'].'" level="'.$var['cat_level'].'"';
 ...
 * @return  mix
 */
function exchangeCategoryList($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;

    if ($res === NULL)
    {
        $data = read_static_cache('exchange_cat_pid_releate');
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['ecs']->table('exchange_category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('exchange_category') . " AS s ON s.parent_id=c.cat_id ".
                "GROUP BY c.cat_id ".
                'ORDER BY c.parent_id, c.sort_order ASC';
            $res = $GLOBALS['db']->getAll($sql);
            $sql = "SELECT cat_id, COUNT(*) AS goods_num  FROM " . $GLOBALS['ecs']->table('exchange_goods') .
                " WHERE is_delete = 0 AND is_on_sale = 1  GROUP BY cat_id";
            $res2 = $GLOBALS['db']->getAll($sql);
            ...
            if (count($res) <= 1000)
            {
                write_static_cache('exchange_cat_pid_releate', $res);
            }
        }
        else
        {
    ...
    $options = exchange_cat_options($cat_id, $res); // 获得指定分类下的子分类的数组
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('exchange_category', array('cid' => $value['cat_id']), $value['cat_name']);
        }
        return $options;
    }
}
 ...
 * @return  void
 */
function exchange_cat_options($spec_cat_id, $arr)
{
    ...
    if (!isset($cat_options[0]))
    {
        ...
        $data = read_static_cache('exchange_cat_option_static');
        if ($data === false)
        {
            while (!empty($arr))
            {
            ...
            {
                write_static_cache('exchange_cat_option_static', $options);
            }
        }
        else
        {
...
function get_children_customize($cat = 0)
{
    return 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat), array_keys(cat_list($cat, 0, false)))));
}
 ...
 * @return  void
 */
function exchange_order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    if (is_null($username))
    {
        $username = $_SESSION['admin_name'];
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('exchange_order_action') .
        ' (order_id, action_user, order_status, shipping_status, pay_status, action_place, action_note, log_time) ' .
        'SELECT ' .
        "order_id, '$username', '$order_status', '$shipping_status', '$pay_status', '$place', '$note', '" .gmtime() . "' " .
        'FROM ' . $GLOBALS['ecs']->table('exchange_order_info') . " WHERE order_sn = '$order_sn'";
    $GLOBALS['db']->query($sql);
}
/**
 * 格式化商品价格
