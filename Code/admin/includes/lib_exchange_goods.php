...
function get_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('exchange_category') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('exchange_category') . " WHERE parent_id = '$parent_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
            'FROM ' . $GLOBALS['ecs']->table('exchange_category') .
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('exchange_category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
        'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS g, ' .
        $GLOBALS['ecs']->table('exchange_order_info') . ' AS o, ' .
        $GLOBALS['ecs']->table('exchange_order_goods') . ' AS og ' .
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
        "FROM " . $GLOBALS['ecs']->table('goods_type') . " AS gt, " . $GLOBALS['ecs']->table('exchange_goods') . " AS g ".
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
        ...
        'LEFT JOIN ' . $GLOBALS['ecs']->table('exchange_attribute') . ' AS a ON a.attr_id = g.attr_id ' .
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                ...
                'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS g ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('exchange_goods_attr') . ' as a ON g.goods_id = a.goods_id ' .
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function spec_price($spec)
{
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $GLOBALS['ecs']->table('exchange_goods_attr') . " WHERE $where";
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
        "FROM " . $GLOBALS['ecs']->table('exchange_goods') . " AS g, " . $GLOBALS['ecs']->table('exchange_attribute') . " AS a " .
        "WHERE g.goods_id = '$goods_id' " .
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
        ...
        'LEFT JOIN ' . $GLOBALS['ecs']->table('exchange_goods') . 'AS g ON g.goods_id = gg.goods_id ' .
        "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
        "ON mp.goods_id = gg.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
        "LEFT JOIN " . $GLOBALS['ecs']->table('exchange_goods') . " AS ggg ON ggg.goods_id = gg.parent_id ".
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('exchange_products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' LIMIT 0, 1";
