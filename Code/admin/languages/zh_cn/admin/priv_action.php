<?php
/* 权限管理的一级分组 */

$_LANG['01_cat_and_goods'] = '基建商城';
$_LANG['02_order'] = '订单管理';
$_LANG['03_exchange'] = "积分商城";
$_LANG['04_wrnews'] = '内容管理';
$_LANG['05_baseData'] = '基础数据管理';
$_LANG['06_credit'] = '信用池管理';
$_LANG['07_customize'] = '定制管理';
$_LANG['08_engineering'] = '工程资讯';
$_LANG['09_service'] = '客服中心';
$_LANG['10_members'] = '会员管理';
$_LANG['11_priv_admin'] = '权限管理';
$_LANG['12_stats'] = '报表统计';
$_LANG['13_contract'] = '合同管理';
$_LANG['14_credit'] = '授信管理';
$_LANG['15_bthPrice'] = '采购价格管理';
$_LANG['16_applyCredit'] = '平台授信管理';
$_LANG['17_bill_manage'] = '应收票据管理';
$_LANG['18_sale_order'] = '销售订单管理';
$_LANG['23_purchase_order_manage'] = '采购订单管理';
$_LANG['19_bank_manage'] = '银行管理';
$_LANG['20_supplier_order'] = '订单管理';
$_LANG['21_order_pay'] = '应付款管理';
$_LANG['22_order_recipient'] = '应收款管理';

//基建商城管理部分的权限
$_LANG['goods_manage'] = '商品添加/编辑';
$_LANG['remove_back'] = '商品删除/恢复';
$_LANG['comment_priv'] = '用户评论管理';
$_LANG['mall_goods_manage'] = '推荐商品';
$_LANG['brand_recommend_manage'] = '品牌推荐';
$_LANG['goods_shipping_set'] = '物流费用设置';

// 订单管理部分权限
$_LANG['order_view'] = '订单处理';
$_LANG['order_edit'] = '订单编辑';

// 积分商城
$_LANG['exchange_category_manage'] = '积分商城分类';
$_LANG['exchange_goods_manage'] = '积分商城商品';
$_LANG['remove_exchange_back'] = '删除积分商品';
$_LANG['exchange_attr_manage'] = '积分商城规格';
$_LANG['exchange_order_view'] = '积分订单管理';
$_LANG['exchange_delivery_view'] = '发货单管理';
$_LANG['exchange_goods_add'] = '添加积分商品';

// 基础数据维护
$_LANG['cat_manage'] = '分类添加/编辑';
$_LANG['cat_drop'] = '分类转移/删除';
$_LANG['attr_manage'] = '商品属性管理';
$_LANG['brand_manage'] = '厂商管理';
$_LANG['area_manage'] = '地区列表管理';
$_LANG['shop_config'] = '积分设置';
$_LANG['suppliers_manage'] = '供货商管理';

// 内容管理
$_LANG['ad_manage'] = '广告管理';
$_LANG['notice_cat'] = '商城公告分类管理';
$_LANG['notice_manage'] = '商城公告';
$_LANG['wrnews_cat'] = '物融新闻分类管理';
$_LANG['wrnews_manage'] = '物融新闻';
$_LANG['helpcenter_cat'] = '帮助中心分类管理';
$_LANG['helpcenter_manage'] = '帮助中心内容';
$_LANG['orderrecommend_manage'] = '推荐订单';

$_LANG['customize_state'] = '物资定制';
$_LANG['custom_state'] = '工程定制';

$_LANG['complaint_state'] = '预约处理';
$_LANG['appointment_state'] = '投诉建议处理';

// 报表统计

$_LANG['credit_analysis_stats'] = '信用分析';
$_LANG['client_flow_stats'] = '流量分析';
$_LANG['client_guest_stats'] = '客户统计';
$_LANG['sale_order_stats'] = '销售统计';
$_LANG['sale_general_stats'] = '销售概况';
$_LANG['sale_list_info_stats'] = '订单明细';
$_LANG['credit_analysis_stats'] = '销售分析';

$_LANG['custom_stats'] = '下游客户对账单';
$_LANG['suppliers_stats'] = '供应商对账单';
$_LANG['contract_stats'] = '项目内部对账单';
// 权限管理
//权限管理部分的权限
$_LANG['users_manage'] = '会员管理';
$_LANG['users_add_total'] = '总账号添加';
$_LANG['admin_manage'] = '管理员添加/编辑';
$_LANG['admin_drop'] = '删除管理员';
$_LANG['logs_manage'] = '管理日志列表';
$_LANG['logs_drop'] = '删除管理日志';
$_LANG['role_manage'] = '角色管理';
//$_LANG['allot_priv'] = '分派权限';

$_LANG['credit_evaluation_state'] = '信用池列表';
$_LANG['credit_quota_add_state'] = '信用申请';
$_LANG['purchase_quota_add_state'] = '采购管理';
$_LANG['credit_intrinfo_manage'] = '信用记录管理';

//合同管理
$_LANG['contract_insert'] = '新增合同';
$_LANG['contract_list'] = '合同列表';
$_LANG['contract_supplier_add'] = '关联供应商、合同';
$_LANG['contract_supplier_list'] = '供应商、合同列表';

//授信管理
$_LANG['credit_list'] = '授信单列表';

//采购价格管理
$_LANG['batch_price'] = '批量加价';
$_LANG['price_list'] = '加价列表';

//平台授信管理
$_LANG['applyCredit_list'] = '平台授信列表';
$_LANG['applyCredit_rlist'] = '平台授信回收站';

//应收票据管理
$_LANG['bill_manage_insert'] = '票据添加';
$_LANG['bill_manage_list'] = '票据列表';
$_LANG['bill_manage_generate'] = '现金额度生成单';
$_LANG['bill_manage_order_list'] = '现金额度生成单列表';
$_LANG['bill_manage_repay_list'] = '票据偿还单列表';
$_LANG['bill_manage_user_list'] = '额度分配调整列表';

//销售订单管理
$_LANG['sale_order_list'] = '（销售订单）订单列表';
$_LANG['sale_order_suborder_all'] = '子订单列表';

//采购订单管理
$_LANG['purchase_order_manage_list'] = '（采购订单）订单列表';

//供应商订单管理
$_LANG['supplier_order_list'] = '（供应商）订单列表';

//应付款管理
$_LANG['order_pay_list'] = '应付单列表';

//应收款管理
$_LANG['order_recipient_comp'] = '已完成的订单';
$_LANG['order_recipient_list'] = '应收单列表';

//银行管理
$_LANG['bank_insert'] = '添加登记机构';
$_LANG['bank_list'] = '登记机构列表';

//审核
$_LANG['reviews'] = '审核';
$_LANG['contract_review'] = '合同审核';
$_LANG['bill_review'] = '票据审核';
$_LANG['bill_amount_review'] = '额度生成单审核';
$_LANG['price_adjust_review'] = '加价规则审核';
?>